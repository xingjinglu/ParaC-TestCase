  ...
  int dst_horizonDstShift)
{
  ...
  /* kernel operation */
  {
    ...
    res1 = res1 >> 4;
    *((__global uchar4 *)(dst_horizonDstDt1)) = convert_uchar4(res1);
  }
...
// Index and Operand for BorderRule 
    ...
    int dst_horizonBRIndex8 = dst_horizonSrcShift + (512- 1) *dst_horizonSrcStep + ((float)gidx * 2) *sizeof(unsigned char);
    int dst_horizonBRIndex6 = dst_horizonSrcShift + (512- 2) *dst_horizonSrcStep + ((float)gidx * 2) *sizeof(unsigned char);
    int dst_horizonBRIndex10 = dst_horizonSrcShift + (512 - 3) *dst_horizonSrcStep + ((float)gidx * 2) *sizeof(unsigned char);
    int dst_horizonBRIndex7 = dst_horizonSrcShift + (512- 4) *dst_horizonSrcStep + ((float)gidx * 2) *sizeof(unsigned char);
  ...
  int layerDstShift)
{
  ...
  // layer[M-1][ity*2]
  //int layerDstIdx3 = layerDstShift + 255 *layerDstStep + ((float)gidx * 2) ;
  //__global unsigned char *layerDstDt3 = layerDst + layerDstIdx3;
  //__global unsigned char *layerDstDt3 = mad24(255, layerDstStep, layerDstShift) + mad24(gidx, 2 , layerDst);
  __global unsigned char *layerDstDt3 = mad24(255, layerDstStep, layerDstShift) + gidx * 2 + layerDst ;
  ...
  __global unsigned char *layerDstDt5 = layerDst + mad24(2, gidx, layerDstShift) + (gidy*2-1)*layerDstStep;

  // layer[2*itx][2*ity] --> layer3
  //int layerDstIdx7 = layerDstShift + (gidy << 1) *layerDstStep + (gidx << 1) ;
  __global unsigned char *layerDstDt7 = layerDst + layerDstShift + (gidy << 1) *layerDstStep + (gidx << 1) ;
  ...
  // dst_ds[(M+1)/2-1][ity]
  int dst_dsSrcIdx9 = dst_dsSrcShift + 127 *dst_dsSrcStep + ((float)gidx) ;
  ...
  dst_ds1.s1 = (short)*((dst_dsSrcDt9+1));
   ...
   int dst_dsSrcIdx14 = dst_dsSrcShift + ((float)gidy + -1) *dst_dsSrcStep + ((float)gidx) ;
  ...
  layerDstIdx1 = layerDstShift + ((float)gidx * 2) ;
  layerDstDt1 = layerDst + layerDstIdx1;

  // layer[0][ity*2+1] 
  layerDstIdx2 = layerDstShift + ((float)gidx * 2 + 1) ;
