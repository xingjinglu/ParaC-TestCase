...
int Laplacian(unsigned char (*Src)[1024][1024], short (*layer)[1024][1024]) {
  ...
  unsigned char dst_ds[(1024 + 1) / 2][(1024 + 1) / 2];
   ...
   unsigned char *src_cpu= (unsigned char *)malloc(1024 * 1024*sizeof(unsigned char*));
  unsigned char *filter_cpu= (unsigned char *)malloc(1024 * 1024*sizeof(unsigned char*));
  unsigned char *down_cpu= (unsigned char *)malloc(512 * 512*sizeof(unsigned char*));
  short *laplacian_cpu= (short *)malloc(1024 * 1024*sizeof(short*));
 
  {
      for(int i=0;i<1024;i++)
          for(int j=0;j<1024;j++)
              *(src_cpu+i*1024+j)= ((unsigned char*)Src)[i*1024+j];

      printf("ss\n");
      PyramidFilterh(src_cpu,filter_cpu, 1024, 1024);
...
//    status = clEnqueueReadBuffer(g_queue, dst_horizondstBuf, CL_TRUE, 0,dst_horizon_dstsz0Pad,dst_horizonDstBufH, 0, NULL, NULL);
    ...
    for(int i = 0; i < 1024; i++)
    {
        for(int j = 0; j < 1024; j++)
        {
            if(filter_cpu[i*1024+j] !=((unsigned char*) dst_horizon)[i* 1024 +j])
            {
                printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)filter_cpu[i*1024+j], (int)((unsigned char*) dst_horizon)[i* 1024 +j]);
    ...
    Downsample(filter_cpu, down_cpu, 1024,1024); 
  ...
  memcpy( (char*)dst_ds+ i *dst_dsDstWidth* sizeof(unsigned char), (char*) dst_dsDstBufH+ (i + PADDING_LINE)* dst_dsDstStep, dst_dsDstWidth* sizeof(unsigned char) );
    ...
    for(int i = 1014; i < 1024; i++)
    {
        for(int j = 0; j < 10; j++)
        {
            ...
            {
             //   printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)filter_cpu[i*1024+j], (int)((unsigned char*) dst_horizon)[i* 256 +j]);
               printf("%d ", (int)filter_cpu[i*1024+j]);
    ...
    for(int i = 0; i < 512; i++)
    {
        for(int j = 0; j < 512; j++)
        {
            if(down_cpu[i*512+j] !=((unsigned char*) dst_ds)[i* 512 +j])
            {
                printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)down_cpu[i*512+j], (int)((unsigned char*) dst_ds)[i* 512 +j]);
                //printf("%u ",yPlaneDown[i][j]);
    ...
    UpsampleX4_reverse(down_cpu, laplacian_cpu, src_cpu, 1024,1024);
  ...
  memcpy( (char*)dst_dsSrcBufH+ (i + PADDING_LINE) * dst_dsSrcStep, (char*)dst_ds+ i * dst_dsSrcWidth* sizeof(unsigned char), dst_dsSrcWidth * sizeof(unsigned char) );
    ...
    checkErr(status, "clCreateBuffer");
    short *layerDstBufH = (short*)malloc(layer_dstsz0Pad); 
  ...
  for(int i = 0; i < 10; i++)
    {
        for(int j = 1014; j < 1024; j++)
        {
            //if(filter_cpu[i*1024+j] !=((unsigned char*) dst_horizon)[i* 1024 +j])
            {
             //   printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)filter_cpu[i*1024+j], (int)((unsigned char*) dst_horizon)[i* 256 +j]);
               printf("%d ", (int)src_cpu[i*1024+j]);
    ...
    for(int i = 0; i < 1024; i++)
    {
        for(int j = 0; j < 1024; j++)
        {
            if(laplacian_cpu[i*1024+j] !=((short*) layer)[i* 1024 +j])
            {
                printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)laplacian_cpu[i*1024+j], (int)((short*) layer)[i* 1024 +j]);
                //printf("%u ",yPlaneDown[i][j]);
...
int main() {
  ...
  short *dst_data = (short *)malloc(sizeof(short) * 1024 * 1024);
  unsigned char (*src)[1024][1024] = (unsigned char (*)[1024][1024])src_data;
  short (*Layer)[1024][1024] = (short (*)[1024][1024])dst_data;
