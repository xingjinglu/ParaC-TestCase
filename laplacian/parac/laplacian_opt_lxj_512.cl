  ...
  int layerDstShift)
{
  ...
  // layer[M-1][ity*2]
  int layerDstIdx3 = layerDstShift + 511 *layerDstStep + ((float)gidx * 2) *sizeof(unsigned char);
  ...
  int SrcSrcIdx3 = SrcSrcShift + 511 *SrcSrcStep + ((float)gidx * 2) *sizeof(unsigned char);
  ...
  // dst_ds[(M+1)/2-1][ity]
  int dst_dsSrcIdx9 = dst_dsSrcShift + 255 *dst_dsSrcStep + ((float)gidx) *sizeof(unsigned char);
