  ...
  int layerDstShift)
{
  ...
  // layer[M-1][ity*2]
  int layerDstIdx3 = layerDstShift + 127 *layerDstStep + ((float)gidx * 2) *sizeof(unsigned char);
  ...
  int SrcSrcIdx3 = SrcSrcShift + 127 *SrcSrcStep + ((float)gidx * 2) *sizeof(unsigned char);
  ...
  // dst_ds[(M+1)/2-1][ity]
  int dst_dsSrcIdx9 = dst_dsSrcShift + 127 *dst_dsSrcStep + ((float)gidx) *sizeof(unsigned char);
