  ...
  int dst_dsDstShift)
{
  ...
  /* kernel operation */
  {
    ...
    int lid = lidy * average *2;

    dsth[lid] = (int) (*((__global uchar*)(dst_horizonSrcDt1 )));
    dsth[lid+1] =(int)  (*((__global uchar*)(dst_horizonSrcDt1 + dst_horizonSrcStep)));
    if( lidy == lsizey-1){
      dsth[lid+4] =(int)  (*((__global uchar*)(dst_horizonSrcDt1 + dst_horizonSrcStep*2 )));
      dsth[lid+5] =(int)  (*((__global uchar*)(dst_horizonSrcDt1 + dst_horizonSrcStep*3 )));
      dsth[lid+8] =(int)  (*((__global uchar*)(dst_horizonSrcDt1 + dst_horizonSrcStep*4 )));
    }
    ...
    if (gidy == 0 ){ 
      ...
      // mid5--> (3, *)
      // dsth[0] --> pre0, dsth[1] --> pre1, dsth[4] --> mid0, dsth[5] = lat0, dsth[8]= lat1
      //pre0 = mid5*4 - lat0*4 + mid0 * 2 - lat1; 
      //pre1 = lat0*2 - mid5;
      dsth[0] = ((dsth[9] - dsth[5])<<2) + (dsth[4]<<1) - dsth[8]; 
      dsth[1] = (dsth[5]<<1) - dsth[9];
    }
    if (gidy ==  511){ 
      ...
      //lat1 = (mid0 << 1) - pre0; 
      dsth[lid+8] = (dsth[lid+4] << 1) - dsth[lid]; 
    }

    //restemp = pre0 + pre1 * 4 +  mid0*6 +  lat0 * 4 + lat1 + 8;
    restemp = dsth[lid] + dsth[lid+1] * 4 + dsth[lid+4]*6 +  dsth[lid+5] * 4 + dsth[lid+8] + 8;
