...
int Laplacian_simd(unsigned char (*Src)[1024][1024], unsigned char (*layer)[1024][1024]) {
  ...
  for(int i = 0; i < 1024; i++){
    ...
    for(int j = 2; j < 1022; j += 8){
      int addr = i * 1024 + j -2 ;
      Src_Src1 = _mm_loadl_epi64( (__m128i const*)((char*)Src + addr  ));
      Src_Src2 = _mm_loadl_epi64( (__m128i const*)((char*)Src + addr + 1));
      Src_Src3 = _mm_loadl_epi64( (__m128i const*)((char*)Src + addr + 2));
      Src_Src4 = _mm_loadl_epi64( (__m128i const*)((char*)Src + addr + 3));
      Src_Src5 = _mm_loadl_epi64( (__m128i const*)((char*)Src + addr + 4));
      ...
      Src_Src2 = _mm_slli_epi16(Src_Src2, 2);
      //Src_Src6 = _mm_slli_epi16(Src_Src3, 1);
      //Src_Src7 = _mm_slli_epi16(Src_Src3, 2);
      Src_Src3 = _mm_slli_epi16(Src_Src3, 1) + _mm_slli_epi16(Src_Src3, 2);
      Src_Src4 = _mm_slli_epi16(Src_Src4, 2);

      Norm = _mm_add_epi16(Norm, Src_Src1);
      Norm = _mm_add_epi16(Norm, Src_Src2);
      Norm = _mm_add_epi16(Norm, Src_Src3);
      Norm = _mm_add_epi16(Norm, Src_Src4);
      Norm = _mm_add_epi16(Norm, Src_Src5);
...
*/
      ...
      Norm = _mm_srli_epi16(Norm, 4);
      Norm = _mm_shuffle_epi8(Norm, Mask1);
      _mm_storeu_si128((__m128i *)&dst_horizon[i][j], Norm);

    }

    res1 =  (int16_t) ((*Src)[i][1020]);
    res1 += (int16_t) ( 4 * (*Src)[i][1021]);
    res1 += (int16_t) ( 6 *  (*Src)[i][1022]);
    res1 += (int16_t) ( 4 * (*Src)[i][1023]);
    res1 += (int16_t) SrcLat1;
    dst_horizon[i][1022] = (unsigned char) ((res1 + 8) / 16);

    //res1 =  (int16_t) ((*Src)[i][1021]);
    //res1 += (int16_t) (4 * (*Src)[i][1022]);
   // res1 += (int16_t) (6 * (*Src)[i][1023]);
    //res1 += (int16_t)4 * SrcLat1;
    //res1 += (int16_t) SrcLat2;
    dst_horizon[i][1023] = (unsigned char) (( ((*Src)[i][1021]) + 
          (4 * (*Src)[i][1022]) +  (6 * (*Src)[i][1023]) +
          4 * SrcLat1 +SrcLat2 
          + 8) / 16);
  ...
  for(int j = 0; j < 512; j += 8){
    ...
    dst_horizonPre1  = _mm_slli_epi16(dst_horizonPre1, 2);
    dst_horizon_Pre2 = _mm_slli_epi16(dst_horizon_Pre2, 2);
      Src_Src6       = _mm_slli_epi16(dst_horizon_Pre1, 1);
      Src_Src7       = _mm_slli_epi16(dst_horizon_Pre1, 2);
    dst_horizon_Pre1 = _mm_add_epi16(Src_Src6, Src_Src7);

    AddRes = _mm_add_epi16(dst_horizonPre2, dst_horizonPre1);
    AddRes = _mm_add_epi16(AddRes, dst_horizon_Pre1);
    AddRes = _mm_add_epi16(AddRes, dst_horizon_Pre2);
    AddRes = _mm_add_epi16(AddRes, dst_horizon_Pre3);

/*
      add = _mm_add_epi16(Src_Src1, Src_Src2);
      add = _mm_add_epi16(add, Src_Src3);
      add = _mm_add_epi16(add, Src_Src4);
      add = _mm_add_epi16(add, Src_Src5);
...
*/
    ...
    dst_horizon_Lat2  = _mm_slli_epi16(dst_horizon_Lat2, 2);
    dst_horizon_Lat4 = _mm_slli_epi16(dst_horizon_Lat4, 2);
      Src_Src6       = _mm_slli_epi16(dst_horizon_Lat3, 1);
      Src_Src7       = _mm_slli_epi16(dst_horizon_Lat3, 2);
    dst_horizon_Lat3 = _mm_add_epi16(Src_Src6, Src_Src7);

    AddRes = _mm_add_epi16(dst_horizon_Lat1, dst_horizon_Lat2);
    AddRes = _mm_add_epi16(AddRes, dst_horizon_Lat3);
    AddRes = _mm_add_epi16(AddRes, dst_horizon_Lat4);
    AddRes = _mm_add_epi16(AddRes, dst_horizonLat1);
    ...
    dst_horizon_Pre4 = _mm_slli_epi16(dst_horizon_Pre4, 2);
    dst_horizon_Pre2 = _mm_slli_epi16(dst_horizon_Pre2, 2);
      Src_Src6       = _mm_slli_epi16(dst_horizon_Pre3, 1);
      Src_Src7       = _mm_slli_epi16(dst_horizon_Pre3, 2);
    dst_horizon_Pre1 = _mm_add_epi16(Src_Src6, Src_Src7);

    AddRes = _mm_add_epi16(dst_horizon_Pre2, dst_horizon_Pre1);
    AddRes = _mm_add_epi16(AddRes, dst_horizon_Pre5);
    AddRes = _mm_add_epi16(AddRes, dst_horizon_Pre4);
    AddRes = _mm_add_epi16(AddRes, dst_horizon_Pre3);
