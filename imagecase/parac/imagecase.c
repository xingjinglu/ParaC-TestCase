...
void resizeImage(char (*mimage)[256][256], char (*dimage)[205][205]) {
  ...
  
  {
    ...
    global_work_size[0] = 204;
    global_work_size[1] = 204;
  ...
  memcpy( (char*)dimageDstBufH+ (i + PADDING_LINE) * dimageDstStep,(char*)dimage + i * dimageDstWidth* sizeof(char),dimageDstWidth* sizeof(char) );
    ...
    status = clEnqueueWriteBuffer(g_queue, dimagedstBuf, CL_TRUE, 0,dimage_dstsz0Pad,dimageDstBufH, 0, NULL, NULL);
    checkErr(status, "clWriteBuffer");
...
int main() {
  char inputfile[] = "imagecase.cl";
  char *remain = NULL;
  if (-1 == openCLCreate(inputfile, remain)) {
    printf("openCL create fail !!!!!!");
    return 0;
  }
  int rROWS = 256 * 0.80000000000000004;
  int rCOLS = 256 * 0.80000000000000004;
  int size = 3;
