...
void init(const unsigned char* _image, int  _image_size)
{
	...
	if(!m_mask)
		std::cout << "m_mask malloc fail" << std::endl;
	...
	if(!(modelW && modelS && modelM))
		std::cout << "GMM modelW or modelS or modelM malloc fail" << std::endl;
...
void processFirstFrame(const unsigned char* _image, int row, int col)
{
	...
	parac_iterator it(iter_element;0: row-1: 1; 0: col - 1: 1)
	{
		...
		modelWData[itx][ity][1: 3: 1] = {0.0f, 0.0f, 0.0f};
		modelSData[itx][ity][1: 3: 1] = {0.0f, 0.0f, 0.0f};
		modelMData[itx][ity][1: 3: 1] = {0, 0, 0};
	}
...
void trainGMM(const unsigned char* _image, int row, int col)
{
	...
	parac_iterator it(it_element; 0: row-1: 1; 0: col - 1: 1)
	{
		...
		long dist = delm * delm;
		if(dist < 3.0 * modelSData[itx][ity][0])
		{
			modelWData[itx][ity][0] += GMM_LEARN_ALPHA * (1 - modelWData[itx][ity][0]);
			modelMData[itx][ity][0] += (GMM_LEARN_ALPHA / modelWData[itx][ity][0]) * delm;
			modelSData[itx][ity][0] += (GMM_LEARN_ALPHA / modelWData[itx][ity][0]) * (dist - modelSData[itx][ity][0]);
		}
		else
		{
			modelWData[itx][ity][0] += GMM_LEARN_ALPHA * (0 - modelWData[itx][ity][0]);
			++num_fit;
		}
		
		delm = abs(_imagData - modelMData[itx][ity][1])
		dist = delm * delm;
		if(dist < 3.0 * modelSData[itx][ity][1])
		{
			modelWData[itx][ity][1] += GMM_LEARN_ALPHA * (1 - modelWData[itx][ity][1]);
			modelMData[itx][ity][1] += (GMM_LEARN_ALPHA / modelWData[itx][ity][1]) * delm;
			modelSData[itx][ity][1] += (GMM_LEARN_ALPHA / modelWData[itx][ity][1]) * (dist - modelSData[itx][ity][1]);
		}
		else
		{
			modelWData[itx][ity][1] += GMM_LEARN_ALPHA * (0 - modelWData[itx][ity][1]);
			++num_fit;
		}
		
		delm = abs(_imagData - modelMData[itx][ity][2])
		dist = delm * delm;
		if(dist < 3.0 * modelSData[itx][ity][2])
		{
			modelWData[itx][ity][2] += GMM_LEARN_ALPHA * (1 - modelWData[itx][ity][2]);
			modelMData[itx][ity][2] += (GMM_LEARN_ALPHA / modelWData[itx][ity][2]) * delm;
			modelSData[itx][ity][2] += (GMM_LEARN_ALPHA / modelWData[itx][ity][2]) * (dist - modelSData[itx][ity][2]);
		}
		else
		{
			modelWData[itx][ity][2] += GMM_LEARN_ALPHA * (0 - modelWData[itx][ity][2]);
			++num_fit;
		}
		
		delm = abs(_imagData - modelMData[itx][ity][3])
		dist = delm * delm;
		if(dist < 3.0 * modelSData[itx][ity][3])
		{
			modelWData[itx][ity][3] += GMM_LEARN_ALPHA * (1 - modelWData[itx][ity][3]);
			modelMData[itx][ity][3] += (GMM_LEARN_ALPHA / modelWData[itx][ity][3]) * delm;
			modelSData[itx][ity][3] += (GMM_LEARN_ALPHA / modelWData[itx][ity][3]) * (dist - modelSData[itx][ity][3]);
		}
		else
		{
			modelWData[itx][ity][3] += GMM_LEARN_ALPHA * (0 - modelWData[itx][ity][3]);
			++num_fit;
		}
