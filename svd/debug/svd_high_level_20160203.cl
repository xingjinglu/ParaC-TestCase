  ...
  int rotate_col_idxDSShift)
{
  ...
  /* kernel operation */
  {
    ...
    ci[1] = (__global double(*)[256])((__global char*)A_HDSDt1 +  256 * A_HDSStep);
    ...
    double ci0R1  = (*cj[0])[lidx];
    double ci0I1  = (*cj[1])[lidx];

    //if(gidx == 4 &&gidy == 0)
     //  printf("ci0R = %f, ci0I=%f, ci0R1=%f, ci0I1 = %f \n",ci0R, ci0I, ci0R1, ci0I1); 


    inner_product(ci0R, ci0I, ci0R1, ci0I1, ld1,ld2,lidx );
    ...
    if (value_abs >= THRESHOLD) {
      ...
      *pass = 0;
      ...
      mid_result[1] = (*ci[1])[lidx] * cos_x + (*cj[1])[lidx] * cos_alpha * sin_x -  (*cj[0])[lidx] * sin_alpha * sin_x;
      mid_result[0] = (*ci[0])[lidx] * cos_x + (*cj[0])[lidx] * cos_alpha * sin_x +  (*cj[1])[lidx] * sin_alpha * sin_x;
      mid_result1[1] = (*cj[1])[lidx] * cos_x - (*ci[1])[lidx] * cos_alpha * sin_x - (*ci[0])[lidx] * sin_alpha * sin_x;
      mid_result1[0] = (*cj[0])[lidx] * cos_x - (*ci[0])[lidx] * cos_alpha * sin_x + (*ci[1])[lidx] * sin_alpha * sin_x;

      //if(gidx == 0 && gidy == 0)
       //   printf("mid0 = %f, mid1= %f, mid2 = %f, mid3= %f\n", mid_result[0],mid_result[1], mid_result1[0], mid_result1[1]);
      ...
      A_HDSDt1 = (__global double*)((__global char*)A_HDS + A_HDSIdx1);
      //TODO:
      //*(A_HDSDt1) = mid_result1[0];
      //*(A_HDSDt1 + A_HDSHeight * A_HDSStep) = mid_result1[1];
      ...
      vi[1] = (__global double(*)[256])((__global char*)V_HDSDt3 +  256 * V_HDSStep);
      ...
      vj[1] = (__global double(*)[256])((__global char*)V_HDSDt4 +  256 * V_HDSStep);
      vj[0] = (__global double(*)[256])( V_HDSDt4 + 0 * 256 * 256);

      double vvv = ((__global double *)((__global char*)V_HDS + V_HDSShift))[0] ;

      //if(gidx == 0 && gidy==0)
       //   printf("mid0 = %f, mid1= %f, mid2 = %f, mid3= %f\n",vvv,(*vi[1])[lidx],(*vj[0])[lidx],(*vj[1])[lidx]);

      mid_result[1] = (*vi[1])[lidx] * cos_x + (*vj[1])[lidx] * cos_alpha * sin_x -  (*vj[0])[lidx] * sin_alpha * sin_x;
      mid_result[0] = (*vi[0])[lidx] * cos_x + (*vj[0])[lidx] * cos_alpha * sin_x +  (*vj[1])[lidx] * sin_alpha * sin_x;
      mid_result1[1] = (*vj[1])[lidx] * cos_x - (*vi[1])[lidx] * cos_alpha * sin_x -  (*vi[0])[lidx] * sin_alpha * sin_x;
      mid_result1[0] = (*vj[0])[lidx] * cos_x - (*vi[0])[lidx] * cos_alpha * sin_x +  (*vi[1])[lidx] * sin_alpha * sin_x;
  ...
  int A_HDSShift)
{
  ...
  int SDstIdx1 = SDstShift + ((float)gridy) *sizeof(double);
  //int A_HDSIdx1 = A_HDSShift + ((float)gridx) *A_HDSStep;
  int A_HDSIdx1 = A_HDSShift + ((float)gridy) *A_HDSStep;
  ...
  /* kernel operation */
  {
    ...
    inner_product(TempVal1R, TempVal1I, STempVal1R, STempVal1I, TempVal1ld1, TempVal1ld2, lidx);
    //TODO
    //double norm_A = sqrt(sqrt( (*TempVal1)[0] * (*TempVal1)[0] + (*TempVal1)[1] * (*TempVal1)[1] ));
    double norm_A = sqrt(sqrt( (TempVal1ld1)[0] * (TempVal1ld1)[0] + (TempVal1ld2)[0] * (TempVal1ld2)[0] ));
    ...
    //TODO
    //(*ci[1]) = (*ci[1])[lidx] / norm_A;
    //(*ci[0]) = (*ci[0])[lidx] / norm_A;
