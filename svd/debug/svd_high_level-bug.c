  ...
  while (iteration > 0)
    {
      for (int i = 0; i < 256; i++) {
        ...
        
        {
                ...
                //TODO: ERROR why global_work_size[0] is so large 
                global_work_size[0] = 18446744073709551615;
        ...
        memcpy((char*)V_HDsBufH + (i + PADDING_LINE) * V_HDSStep, (char*)V_H+ i * V_HDSWidth * sizeof(double) * 2, V_HDSWidth* sizeof(double) * 2 );
                ...
                Readsize = rotate_col_idxDSWidth * rotate_col_idx_dssz;
                ...
                checkErr(status, "clSetKernelArg");
                //TOOD: local_group_size must be figured
                ...
                //TODO:matirx A and V is not necessarily  copied to CPU in this place, these operation are only need to do outside the while loop
                //However, variable pass must be copied to CPU at the end of for loop. 
        ...
        status = clEnqueueReadBuffer(g_queue, rotate_col_idxdsBuf, CL_TRUE, 0,rotate_col_idx_dssz0Pad,rotate_col_idxDsBufH, 0, NULL, NULL);
                ...
                for(int i = 0; i < A_HDSHeight; i++){
        memcpy( (char*)A_H+ i *A_HDSWidth* sizeof(double), (char*) A_HDsBufH+ (i + PADDING_LINE)* A_HDSStep, A_HDSWidth* sizeof(double) * 2 );
        }
        for(int i = 0; i < V_HDSHeight; i++){
        memcpy( (char*)V_H+ i *V_HDSWidth* sizeof(double), (char*) V_HDsBufH+ (i + PADDING_LINE)* V_HDSStep, V_HDSWidth* sizeof(double) * 2 );
        }
