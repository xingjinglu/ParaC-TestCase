  ...
  for (int ii = 0; ii < 10; ii++) {
    ...
    #pragma cap data copyin(Source[0:width * height]) create(FilterDst[0:width * height], pSrc_padding[0:(width + 4) * (height + 4)], pSrc_padding2[0:(width + 4) * (height + 4)], pBufL_cp[0:width * height]) copyout(LaplacianLayer[0:width * height]) 
      {
        ...
        //set kernel parameters
        status = clSetKernelArg(kernel_2, 0, sizeof(cl_mem), (void *)&pSrc_padding20Buffer);
        checkErr(status, "clSetKernelArg for 0");
        ...
        checkErr(status, "clSetKernelArg for 3");
        status = clSetKernelArg(kernel_2, 4, sizeof(cl_mem), (void *)&pBufL_cp0Buffer);
        ...
        blockWorkSize3[0] = upMultiple( (width + 2) - (2), 256 );
        ...
        localWorkSize4[0] = 32;
        blockWorkSize4[0] = upMultiple( width, 32 );
        localWorkSize4[1] = 8;
        blockWorkSize4[1] = upMultiple( height, 8 );
        globalWorkSize4[0] = blockWorkSize4[0] * localWorkSize4[0];
        globalWorkSize4[1] = blockWorkSize4[1] * localWorkSize4[1];
        //set kernel parameters
        status = clSetKernelArg(kernel_4, 0, sizeof(cl_mem), (void *)&FilterDst0Buffer);
        checkErr(status, "clSetKernelArg for 0");
        status = clSetKernelArg(kernel_4, 1, sizeof(int), (void *)&height);
        checkErr(status, "clSetKernelArg for 1");
        status = clSetKernelArg(kernel_4, 2, sizeof(int), (void *)&width);
        checkErr(status, "clSetKernelArg for 2");
        status = clSetKernelArg(kernel_4, 3, sizeof(cl_mem), (void *)&pSrc_padding20Buffer);
        checkErr(status, "clSetKernelArg for 3");

        //launch a kernel
        status = clEnqueueNDRangeKernel(queue, kernel_4, 2, NULL, globalWorkSize4, localWorkSize4, 0, NULL, NULL);
        ...
        #pragma cap data copyout(DownsampleDst[0:halfHeight * halfWidth]) 
          {
            ...
            localWorkSize5[1] = 8;
            blockWorkSize5[1] = upMultiple( halfHeight, 8 );
            localWorkSize5[0] = 32;
            blockWorkSize5[0] = upMultiple( halfWidth, 32 );
            globalWorkSize5[0] = blockWorkSize5[0] * localWorkSize5[0];
            globalWorkSize5[1] = blockWorkSize5[1] * localWorkSize5[1];
            //set kernel parameters
            status = clSetKernelArg(kernel_5, 0, sizeof(cl_mem), (void *)&DownsampleDst1Buffer);
            checkErr(status, "clSetKernelArg for 0");
            status = clSetKernelArg(kernel_5, 1, sizeof(int), (void *)&width);
            checkErr(status, "clSetKernelArg for 1");
            status = clSetKernelArg(kernel_5, 2, sizeof(int), (void *)&halfWidth);
            checkErr(status, "clSetKernelArg for 2");
            status = clSetKernelArg(kernel_5, 3, sizeof(int), (void *)&halfHeight);
            checkErr(status, "clSetKernelArg for 3");
            status = clSetKernelArg(kernel_5, 4, sizeof(cl_mem), (void *)&FilterDst0Buffer);
            checkErr(status, "clSetKernelArg for 4");
        ...
        #pragma cap data copyin(DownsampleDst[0:halfHeight * halfWidth], UpsampleDst[0:width * height]) 
          {
            ...
            blockWorkSize6[0] = upMultiple( halfWidth - (1), 256 );
            globalWorkSize6[0] = blockWorkSize6[0] * localWorkSize6[0];
            //set kernel parameters
            status = clSetKernelArg(kernel_6, 0, sizeof(cl_mem), (void *)&UpsampleDst2Buffer);
            checkErr(status, "clSetKernelArg for 0");
            status = clSetKernelArg(kernel_6, 1, sizeof(int), (void *)&height);
            checkErr(status, "clSetKernelArg for 1");
            status = clSetKernelArg(kernel_6, 2, sizeof(int), (void *)&width);
            checkErr(status, "clSetKernelArg for 2");
            status = clSetKernelArg(kernel_6, 3, sizeof(int), (void *)&halfWidth);
            checkErr(status, "clSetKernelArg for 3");
            status = clSetKernelArg(kernel_6, 4, sizeof(int), (void *)&halfHeight);
            checkErr(status, "clSetKernelArg for 4");
            status = clSetKernelArg(kernel_6, 5, sizeof(cl_mem), (void *)&DownsampleDst2Buffer);
            ...
            localWorkSize7[1] = 8;
            blockWorkSize7[1] = upMultiple( (0 - (halfHeight - 1)) * -1, 8 );
            localWorkSize7[0] = 32;
            blockWorkSize7[0] = upMultiple( halfWidth - (1), 32 );
            globalWorkSize7[0] = blockWorkSize7[0] * localWorkSize7[0];
            globalWorkSize7[1] = blockWorkSize7[1] * localWorkSize7[1];
            //set kernel parameters
            status = clSetKernelArg(kernel_7, 0, sizeof(cl_mem), (void *)&UpsampleDst2Buffer);
            checkErr(status, "clSetKernelArg for 0");
            status = clSetKernelArg(kernel_7, 1, sizeof(int), (void *)&width);
            checkErr(status, "clSetKernelArg for 1");
            status = clSetKernelArg(kernel_7, 2, sizeof(int), (void *)&halfWidth);
            checkErr(status, "clSetKernelArg for 2");
            status = clSetKernelArg(kernel_7, 3, sizeof(int), (void *)&halfHeight);
            checkErr(status, "clSetKernelArg for 3");
            status = clSetKernelArg(kernel_7, 4, sizeof(cl_mem), (void *)&DownsampleDst2Buffer);
            checkErr(status, "clSetKernelArg for 4");

            //launch a kernel
            status = clEnqueueNDRangeKernel(queue, kernel_7, 2, NULL, globalWorkSize7, localWorkSize7, 0, NULL, NULL);
            ...
            localWorkSize8[1] = 8;
            blockWorkSize8[1] = upMultiple( height, 8 );
            localWorkSize8[0] = 32;
            blockWorkSize8[0] = upMultiple( width, 32 );
            globalWorkSize8[0] = blockWorkSize8[0] * localWorkSize8[0];
            globalWorkSize8[1] = blockWorkSize8[1] * localWorkSize8[1];
            //set kernel parameters
            status = clSetKernelArg(kernel_8, 0, sizeof(cl_mem), (void *)&LaplacianLayer0Buffer);
            checkErr(status, "clSetKernelArg for 0");
            status = clSetKernelArg(kernel_8, 1, sizeof(int), (void *)&height);
            checkErr(status, "clSetKernelArg for 1");
            status = clSetKernelArg(kernel_8, 2, sizeof(int), (void *)&width);
            checkErr(status, "clSetKernelArg for 2");
            status = clSetKernelArg(kernel_8, 3, sizeof(cl_mem), (void *)&Source0Buffer);
            checkErr(status, "clSetKernelArg for 3");
            status = clSetKernelArg(kernel_8, 4, sizeof(cl_mem), (void *)&UpsampleDst2Buffer);
            checkErr(status, "clSetKernelArg for 4");
    ...
    for (int i = 0; i < 7; i++) {
      ...
      if (i == 0)
        ...
        printf("Row_filter+Column_padding: ");
      else if (i == 3)
...
int main(int argc, char **argv) {
  ...
  const char *inputFile = "laplacian_fuse-cap.cl";
