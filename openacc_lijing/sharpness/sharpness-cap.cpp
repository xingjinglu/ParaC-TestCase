    ...
    #pragma cap data copy(yPlane[0:width * height]) create(yPlaneCSER[0:width * height], yPlaneDown[0:quarterHeight * quarterWidth], pError[0:width * height], pEdge[0:width * height], pSharpness[0:width * height]) 
      {
        ...
        checkErr(status, "clSetKernelArg for 1");
        status = clSetKernelArg(kernel_5, 2, sizeof(int), (void *)&quarterWidth);
        checkErr(status, "clSetKernelArg for 2");
        status = clSetKernelArg(kernel_5, 3, sizeof(cl_mem), (void *)&yPlaneDown0Buffer);
        checkErr(status, "clSetKernelArg for 3");
        ...
        checkErr(status, "clSetKernelArg for 0");
        status = clSetKernelArg(kernel_6, 1, sizeof(int), (void *)&quarterWidth);
        checkErr(status, "clSetKernelArg for 1");
        status = clSetKernelArg(kernel_6, 2, sizeof(cl_mem), (void *)&yPlaneCSER0Buffer);
        checkErr(status, "clSetKernelArg for 2");
        ...
        checkErr(status, "clSetKernelArg for 0");
        status = clSetKernelArg(kernel_7, 1, sizeof(cl_mem), (void *)&yPlaneCSER0Buffer);
        checkErr(status, "clSetKernelArg for 1");
        ...
        blockWorkSize8[0] = upMultiple( quarterWidth, 256 );
        ...
        checkErr(status, "clSetKernelArg for 1");
        status = clSetKernelArg(kernel_8, 2, sizeof(int), (void *)&height);
        checkErr(status, "clSetKernelArg for 2");
        status = clSetKernelArg(kernel_8, 3, sizeof(int), (void *)&quarterWidth);
        checkErr(status, "clSetKernelArg for 3");
        status = clSetKernelArg(kernel_8, 4, sizeof(int), (void *)&quarterHeight);
        checkErr(status, "clSetKernelArg for 4");
        status = clSetKernelArg(kernel_8, 5, sizeof(cl_mem), (void *)&yPlaneDown0Buffer);
        ...
        blockWorkSize9[0] = upMultiple( quarterWidth, 256 );
        globalWorkSize9[0] = blockWorkSize9[0] * localWorkSize9[0];
        //set kernel parameters
        status = clSetKernelArg(kernel_9, 0, sizeof(int), (void *)&width);
        checkErr(status, "clSetKernelArg for 0");
        status = clSetKernelArg(kernel_9, 1, sizeof(int), (void *)&height);
        checkErr(status, "clSetKernelArg for 1");
        status = clSetKernelArg(kernel_9, 2, sizeof(int), (void *)&quarterWidth);
        checkErr(status, "clSetKernelArg for 2");
        status = clSetKernelArg(kernel_9, 3, sizeof(cl_mem), (void *)&yPlaneCSER0Buffer);
        checkErr(status, "clSetKernelArg for 3");
        ...
        blockWorkSize10[0] = upMultiple( width, 256 );
        ...
        localWorkSize11[0] = 256;
        blockWorkSize11[0] = upMultiple( quarterHeight, 256 );
        ...
        //launch a kernel
        status = clEnqueueNDRangeKernel(queue, kernel_11, 1, NULL, globalWorkSize11, localWorkSize11, 0, NULL, NULL);
        ...
        localWorkSize12[0] = 256;
        blockWorkSize12[0] = upMultiple( quarterHeight, 256 );
        globalWorkSize12[0] = blockWorkSize12[0] * localWorkSize12[0];
        //set kernel parameters
        status = clSetKernelArg(kernel_12, 0, sizeof(int), (void *)&width);
        checkErr(status, "clSetKernelArg for 0");
        status = clSetKernelArg(kernel_12, 1, sizeof(int), (void *)&quarterHeight);
        checkErr(status, "clSetKernelArg for 1");
        status = clSetKernelArg(kernel_12, 2, sizeof(cl_mem), (void *)&yPlaneCSER0Buffer);
        checkErr(status, "clSetKernelArg for 2");

        //launch a kernel
        status = clEnqueueNDRangeKernel(queue, kernel_12, 1, NULL, globalWorkSize12, localWorkSize12, 0, NULL, NULL);
        ...
        localWorkSize13[0] = 256;
        blockWorkSize13[0] = upMultiple( height, 256 );
        ...
        checkErr(status, "clSetKernelArg for 1");
        status = clSetKernelArg(kernel_13, 2, sizeof(cl_mem), (void *)&yPlaneCSER0Buffer);
        checkErr(status, "clSetKernelArg for 2");

        //launch a kernel
        status = clEnqueueNDRangeKernel(queue, kernel_13, 1, NULL, globalWorkSize13, localWorkSize13, 0, NULL, NULL);
        ...
        localWorkSize14[1] = 8;
        blockWorkSize14[1] = upMultiple( quarterHeight - 1, 8 );
        localWorkSize14[0] = 32;
        blockWorkSize14[0] = upMultiple( quarterWidth - 1, 32 );
        globalWorkSize14[0] = blockWorkSize14[0] * localWorkSize14[0];
        globalWorkSize14[1] = blockWorkSize14[1] * localWorkSize14[1];
        //set kernel parameters
        status = clSetKernelArg(kernel_14, 0, sizeof(cl_mem), (void *)&yPlaneCSER0Buffer);
        checkErr(status, "clSetKernelArg for 0");
        status = clSetKernelArg(kernel_14, 1, sizeof(int), (void *)&width);
        checkErr(status, "clSetKernelArg for 1");
        status = clSetKernelArg(kernel_14, 2, sizeof(int), (void *)&quarterWidth);
        checkErr(status, "clSetKernelArg for 2");
        status = clSetKernelArg(kernel_14, 3, sizeof(int), (void *)&quarterHeight);
        checkErr(status, "clSetKernelArg for 3");
        status = clSetKernelArg(kernel_14, 4, sizeof(cl_mem), (void *)&yPlaneDown0Buffer);
        checkErr(status, "clSetKernelArg for 4");
        ...
        gettimeofday(&start_cpu[3], __null);
                //create the kernel
        cl_kernel kernel_15 = clCreateKernel(program, "kernel_15", &status);
        checkErr(status, "clCreateKernel for kernel_15");

        //set thread/thread-block size
        size_t localWorkSize15[3]= {1, 1, 1};
        size_t blockWorkSize15[3]= {1, 1, 1};
        size_t globalWorkSize15[3]= {1, 1, 1};
        localWorkSize15[1] = 8;
        blockWorkSize15[1] = upMultiple( height, 8 );
        localWorkSize15[0] = 32;
        blockWorkSize15[0] = upMultiple( width, 32 );
        globalWorkSize15[0] = blockWorkSize15[0] * localWorkSize15[0];
        globalWorkSize15[1] = blockWorkSize15[1] * localWorkSize15[1];
        //set kernel parameters
        status = clSetKernelArg(kernel_15, 0, sizeof(cl_mem), (void *)&pError0Buffer);
        checkErr(status, "clSetKernelArg for 0");
        status = clSetKernelArg(kernel_15, 1, sizeof(int), (void *)&width);
        checkErr(status, "clSetKernelArg for 1");
        status = clSetKernelArg(kernel_15, 2, sizeof(int), (void *)&height);
        checkErr(status, "clSetKernelArg for 2");
        status = clSetKernelArg(kernel_15, 3, sizeof(cl_mem), (void *)&yPlane0Buffer);
        checkErr(status, "clSetKernelArg for 3");
        status = clSetKernelArg(kernel_15, 4, sizeof(cl_mem), (void *)&yPlaneCSER0Buffer);
        checkErr(status, "clSetKernelArg for 4");

        //launch a kernel
        status = clEnqueueNDRangeKernel(queue, kernel_15, 2, NULL, globalWorkSize15, localWorkSize15, 0, NULL, NULL);
        checkErr(status, "clEnqueueNDRangeKernel for kernel_15");

        //wait until kernel finish
        status = clFinish(queue);
        checkErr(status, "clFinish for kernel_15");

        //release kernel
        clReleaseKernel(kernel_15);
        gettimeofday(&end_cpu[3], __null);
        gettimeofday(&start_cpu[4], __null);
                //create the kernel
        cl_kernel kernel_16 = clCreateKernel(program, "kernel_16", &status);
        checkErr(status, "clCreateKernel for kernel_16");

        //set thread/thread-block size
        size_t localWorkSize16[3]= {1, 1, 1};
        size_t blockWorkSize16[3]= {1, 1, 1};
        size_t globalWorkSize16[3]= {1, 1, 1};
        localWorkSize16[1] = 8;
        blockWorkSize16[1] = upMultiple( height, 8 );
        localWorkSize16[0] = 32;
        blockWorkSize16[0] = upMultiple( width, 32 );
        globalWorkSize16[0] = blockWorkSize16[0] * localWorkSize16[0];
        globalWorkSize16[1] = blockWorkSize16[1] * localWorkSize16[1];
        //set kernel parameters
        status = clSetKernelArg(kernel_16, 0, sizeof(cl_mem), (void *)&pEdge0Buffer);
        checkErr(status, "clSetKernelArg for 0");
        status = clSetKernelArg(kernel_16, 1, sizeof(int), (void *)&width);
        checkErr(status, "clSetKernelArg for 1");
        status = clSetKernelArg(kernel_16, 2, sizeof(int), (void *)&height);
        checkErr(status, "clSetKernelArg for 2");
        status = clSetKernelArg(kernel_16, 3, sizeof(cl_mem), (void *)&yPlane0Buffer);
        checkErr(status, "clSetKernelArg for 3");

        //launch a kernel
        status = clEnqueueNDRangeKernel(queue, kernel_16, 2, NULL, globalWorkSize16, localWorkSize16, 0, NULL, NULL);
        checkErr(status, "clEnqueueNDRangeKernel for kernel_16");

        //wait until kernel finish
        status = clFinish(queue);
        checkErr(status, "clFinish for kernel_16");

        //release kernel
        clReleaseKernel(kernel_16);
        gettimeofday(&end_cpu[4], __null);
        gettimeofday(&start_cpu[5], __null);
                //create reduction buffers on GPU 
        cl_mem sum20Buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(long), NULL, &status);
        checkErr(status, "clCreateBuffer for sum");

        //copyin reduction buffers to GPU 
        status = clEnqueueWriteBuffer(queue, sum20Buffer, CL_TRUE, 0, sizeof(long), &sum, 0, NULL, NULL);
        checkErr(status, "clEnqueueWriteBuffer for sum");

        //create the kernel
        cl_kernel kernel_17 = clCreateKernel(program, "kernel_17", &status);
        checkErr(status, "clCreateKernel for kernel_17");

        //set thread/thread-block size
        size_t localWorkSize17[3]= {1, 1, 1};
        size_t blockWorkSize17[3]= {1, 1, 1};
        size_t globalWorkSize17[3]= {1, 1, 1};
        localWorkSize17[1] = 8;
        blockWorkSize17[1] = upMultiple( height, 8 );
        localWorkSize17[0] = 32;
        blockWorkSize17[0] = upMultiple( width, 32 );
        globalWorkSize17[0] = blockWorkSize17[0] * localWorkSize17[0];
        globalWorkSize17[1] = blockWorkSize17[1] * localWorkSize17[1];
        //set kernel parameters
        status = clSetKernelArg(kernel_17, 0, sizeof(int), (void *)&width);
        checkErr(status, "clSetKernelArg for 0");
        status = clSetKernelArg(kernel_17, 1, sizeof(int), (void *)&height);
        checkErr(status, "clSetKernelArg for 1");
        status = clSetKernelArg(kernel_17, 2, sizeof(cl_mem), (void *)&pEdge0Buffer);
        checkErr(status, "clSetKernelArg for 2");
        status = clSetKernelArg(kernel_17, 3, sizeof(cl_mem), (void *)&sum20Buffer);
        checkErr(status, "clSetKernelArg for 3");
        status = clSetKernelArg(kernel_17, 4, 8 * localWorkSize17[0] * localWorkSize17[1] * localWorkSize17[2] , NULL);
        checkErr(status, "clSetKernelArg for 4");

        //launch a kernel
        status = clEnqueueNDRangeKernel(queue, kernel_17, 2, NULL, globalWorkSize17, localWorkSize17, 0, NULL, NULL);
        checkErr(status, "clEnqueueNDRangeKernel for kernel_17");

        //wait until kernel finish
        status = clFinish(queue);
        checkErr(status, "clFinish for kernel_17");

        //release kernel
        clReleaseKernel(kernel_17);
//copyout reduction buffers to CPU 
        status = clEnqueueReadBuffer(queue, sum20Buffer, CL_TRUE, 0, sizeof(long), &sum, 0, NULL, NULL);
        checkErr(status, "clEnqueueReadBuffer for sum");

        //release reduction buffers on GPU
        clReleaseMemObject(sum20Buffer);
        ...
        gettimeofday(&start_cpu[6], __null);
                //create the kernel
        cl_kernel kernel_18 = clCreateKernel(program, "kernel_18", &status);
        checkErr(status, "clCreateKernel for kernel_18");

        //set thread/thread-block size
        size_t localWorkSize18[3]= {1, 1, 1};
        size_t blockWorkSize18[3]= {1, 1, 1};
        size_t globalWorkSize18[3]= {1, 1, 1};
        localWorkSize18[1] = 8;
        blockWorkSize18[1] = upMultiple( height, 8 );
        localWorkSize18[0] = 32;
        blockWorkSize18[0] = upMultiple( width, 32 );
        globalWorkSize18[0] = blockWorkSize18[0] * localWorkSize18[0];
        globalWorkSize18[1] = blockWorkSize18[1] * localWorkSize18[1];
        //set kernel parameters
        status = clSetKernelArg(kernel_18, 0, sizeof(int), (void *)&width);
        checkErr(status, "clSetKernelArg for 0");
        status = clSetKernelArg(kernel_18, 1, sizeof(int), (void *)&height);
        checkErr(status, "clSetKernelArg for 1");
        status = clSetKernelArg(kernel_18, 2, sizeof(cl_mem), (void *)&yPlaneCSER0Buffer);
        checkErr(status, "clSetKernelArg for 2");
        status = clSetKernelArg(kernel_18, 3, sizeof(cl_mem), (void *)&pError0Buffer);
        checkErr(status, "clSetKernelArg for 3");
        status = clSetKernelArg(kernel_18, 4, sizeof(cl_mem), (void *)&pEdge0Buffer);
        checkErr(status, "clSetKernelArg for 4");
        status = clSetKernelArg(kernel_18, 5, sizeof(double), (void *)&cutoff);
        checkErr(status, "clSetKernelArg for 5");
        status = clSetKernelArg(kernel_18, 6, sizeof(double), (void *)&peak);
        checkErr(status, "clSetKernelArg for 6");
        status = clSetKernelArg(kernel_18, 7, sizeof(double), (void *)&cutoff2);
        checkErr(status, "clSetKernelArg for 7");
        status = clSetKernelArg(kernel_18, 8, sizeof(double), (void *)&order);
        checkErr(status, "clSetKernelArg for 8");
        status = clSetKernelArg(kernel_18, 9, sizeof(double), (void *)&order2);
        checkErr(status, "clSetKernelArg for 9");
        status = clSetKernelArg(kernel_18, 10, sizeof(double), (void *)&low);
        checkErr(status, "clSetKernelArg for 10");
        status = clSetKernelArg(kernel_18, 11, sizeof(cl_mem), (void *)&pSharpness0Buffer);
        checkErr(status, "clSetKernelArg for 11");

        //launch a kernel
        status = clEnqueueNDRangeKernel(queue, kernel_18, 2, NULL, globalWorkSize18, localWorkSize18, 0, NULL, NULL);
        checkErr(status, "clEnqueueNDRangeKernel for kernel_18");

        //wait until kernel finish
        status = clFinish(queue);
        checkErr(status, "clFinish for kernel_18");

        //release kernel
        clReleaseKernel(kernel_18);
        gettimeofday(&end_cpu[6], __null);
        gettimeofday(&start_cpu[7], __null);
                //create the kernel
        cl_kernel kernel_19 = clCreateKernel(program, "kernel_19", &status);
        checkErr(status, "clCreateKernel for kernel_19");

        //set thread/thread-block size
        size_t localWorkSize19[3]= {1, 1, 1};
        size_t blockWorkSize19[3]= {1, 1, 1};
        size_t globalWorkSize19[3]= {1, 1, 1};
        localWorkSize19[1] = 8;
        blockWorkSize19[1] = upMultiple( height, 8 );
        localWorkSize19[0] = 32;
        blockWorkSize19[0] = upMultiple( width, 32 );
        globalWorkSize19[0] = blockWorkSize19[0] * localWorkSize19[0];
        globalWorkSize19[1] = blockWorkSize19[1] * localWorkSize19[1];
        //set kernel parameters
        status = clSetKernelArg(kernel_19, 0, sizeof(int), (void *)&width);
        checkErr(status, "clSetKernelArg for 0");
        status = clSetKernelArg(kernel_19, 1, sizeof(int), (void *)&height);
        checkErr(status, "clSetKernelArg for 1");
        status = clSetKernelArg(kernel_19, 2, sizeof(cl_mem), (void *)&yPlane0Buffer);
        checkErr(status, "clSetKernelArg for 2");
        status = clSetKernelArg(kernel_19, 3, sizeof(cl_mem), (void *)&pEdge0Buffer);
        checkErr(status, "clSetKernelArg for 3");
        status = clSetKernelArg(kernel_19, 4, sizeof(float), (void *)&cutoff3);
        checkErr(status, "clSetKernelArg for 4");
        status = clSetKernelArg(kernel_19, 5, sizeof(float), (void *)&order3);
        checkErr(status, "clSetKernelArg for 5");
        status = clSetKernelArg(kernel_19, 6, sizeof(float), (void *)&peak3);
        checkErr(status, "clSetKernelArg for 6");
        status = clSetKernelArg(kernel_19, 7, sizeof(float), (void *)&low3);
        checkErr(status, "clSetKernelArg for 7");
        status = clSetKernelArg(kernel_19, 8, sizeof(cl_mem), (void *)&pSharpness0Buffer);
        checkErr(status, "clSetKernelArg for 8");

        //launch a kernel
        status = clEnqueueNDRangeKernel(queue, kernel_19, 2, NULL, globalWorkSize19, localWorkSize19, 0, NULL, NULL);
        checkErr(status, "clEnqueueNDRangeKernel for kernel_19");

        //wait until kernel finish
        status = clFinish(queue);
        checkErr(status, "clFinish for kernel_19");

        //release kernel
        clReleaseKernel(kernel_19);
        gettimeofday(&end_cpu[7], __null);
        gettimeofday(&start_cpu[8], __null);
                //create the kernel
        cl_kernel kernel_20 = clCreateKernel(program, "kernel_20", &status);
        checkErr(status, "clCreateKernel for kernel_20");

        //set thread/thread-block size
        size_t localWorkSize20[3]= {1, 1, 1};
        size_t blockWorkSize20[3]= {1, 1, 1};
        size_t globalWorkSize20[3]= {1, 1, 1};
        localWorkSize20[1] = 8;
        blockWorkSize20[1] = upMultiple( height, 8 );
        localWorkSize20[0] = 32;
        blockWorkSize20[0] = upMultiple( width, 32 );
        globalWorkSize20[0] = blockWorkSize20[0] * localWorkSize20[0];
        globalWorkSize20[1] = blockWorkSize20[1] * localWorkSize20[1];
        //set kernel parameters
        status = clSetKernelArg(kernel_20, 0, sizeof(cl_mem), (void *)&yPlane0Buffer);
        checkErr(status, "clSetKernelArg for 0");
        status = clSetKernelArg(kernel_20, 1, sizeof(int), (void *)&width);
        checkErr(status, "clSetKernelArg for 1");
        status = clSetKernelArg(kernel_20, 2, sizeof(int), (void *)&height);
        checkErr(status, "clSetKernelArg for 2");
        status = clSetKernelArg(kernel_20, 3, sizeof(cl_mem), (void *)&pSharpness0Buffer);
        checkErr(status, "clSetKernelArg for 3");

        //launch a kernel
        status = clEnqueueNDRangeKernel(queue, kernel_20, 2, NULL, globalWorkSize20, localWorkSize20, 0, NULL, NULL);
        checkErr(status, "clEnqueueNDRangeKernel for kernel_20");

        //wait until kernel finish
        status = clFinish(queue);
        checkErr(status, "clFinish for kernel_20");

        //release kernel
        clReleaseKernel(kernel_20);
...
int main(int argc, char *argv[]) {
  ...
  const char *inputFile = "sharpness-cap.cl";
