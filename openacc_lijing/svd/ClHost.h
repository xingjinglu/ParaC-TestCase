...
//ceil(divident/divisor)
int upMultiple(int dividend, int divisor)
{
   int temp0 = dividend%divisor;
   int temp1 = dividend/divisor;
   return temp0==0 ? temp1 : (temp1+1);
}

//print the error message for err
...
//create OpenCL context, queue, and program for a given inputfile name
int openCLCreate(cl_context* context,cl_command_queue* queue, cl_program* program, const char* inputfile)
{
   cl_int status;
   ...
   if(numPlatforms>0)
   {
	 ...
	 return -1;
      ...
      //use the first platform by default
