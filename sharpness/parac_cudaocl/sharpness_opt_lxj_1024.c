...
int UpScaleNewX16(int height, int width, unsigned char *srcImage1, unsigned char *dstImage1) 
{
...

  {
    ...
    for(int i = 0; i< 20; i++)
    {
      ...
      //status = clEnqueueNDRangeKernel(g_queue, kernel_4, 2, NULL, global_work_size, local_work_size, 0, NULL, &event_kernel);
      checkErr(status, "clEnqueueNDRangeKernel");
      status = clFinish(g_queue);
  ...
  unsigned char (*dst)[height][width] = (unsigned char(*)[height][width])(dst3);
...

    {
        ...
        for(int i = 0; i< 20; i++)
        {
            ...
            status = clEnqueueNDRangeKernel(g_queue, kernel_5, 2, NULL, global_work_size, local_work_size, 0, NULL, &event_kernel);
            checkErr(status, "clEnqueueNDRangeKernel");
            status = clFinish(g_queue);
    ...
    total.sobel_time = (double)(tem/20.0);
    total.kernel_sobel = (double)(tem1/20000.0);
...
#ifdef TIME_PROF
    total.mean_time = tem / 20.0;
    total.kernel_mean =(double)( tem1 / 20000.0);
    ...
    total.sharp_time = tem / 20.0;
    total.kernel_sharpness = (double)(tem1 / 20000.0);
...
int main() {
    char *inputfile = "sharpness_opt_lxj_1024.cl";
    char *remain = NULL;
