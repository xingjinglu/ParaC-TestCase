...
int DownScaleNewX16(unsigned char (*pImageSrc)[2048][2048], unsigned char (*pImageDst)[2048 / 4][2048 / 4]) {

    unsigned char **yPlane=allocu(2048,2048);
    unsigned char** yPlaneDown=allocu(512,512);

    for(int i=0;i<2048;i++)
        for(int j=0;j<2048;j++)
            yPlane[i][j]=((unsigned char*)pImageSrc)[i*2048+j];

    printf("ss\n");
    DownScaleNewX16_cpu(yPlane,512, 512,  yPlaneDown);

    {
        tem = 0;
        for(int i = 0; i< 20; i++)
        {
            size_t pImageSrcSrcWidth = 2048;
            size_t pImageSrcSrcHeight = 2048;
            ...
            checkErr(status, "clSetKernelArg");
            size_t pImageDstDstWidth = 512;
            size_t pImageDstDstHeight = 512;
   ...
   printf("Down : %f ms\n",tem/20);
    ...
    for(int i = 0; i < 512; i++)
    {
        for(int j = 0; j < 512; j++)
        {
            if(yPlaneDown[i][j] !=((unsigned char*) pImageDst)[i* 512 +j])
            {
                printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)yPlaneDown[i][j], (int)((unsigned char*) pImageDst)[i* 512 +j]);
...
int UpScaleNewX16(unsigned char (*srcImage)[2048 / 4][2048 / 4], unsigned char (*dstImage)[2048][2048]) {

    unsigned char **yPlane=allocu(512,512);
    unsigned char** yPlaneUp=allocu(2048,2048);

    for(int i=0;i<512;i++)
        for(int j=0;j<512;j++)
            yPlane[i][j]=((unsigned char*)srcImage)[i*512+j];

    printf("ss\n");
    UpScaleNewX16_cpu(yPlane,512, 512, yPlaneUp);
...

    {
        ...
        for(int i = 0; i< 20 ; i++)
        {
            size_t srcImageSrcWidth = 512;
            size_t srcImageSrcHeight = 512;
            ...
            size_t dstImageDSWidth = 2048;
            size_t dstImageDSHeight = 2048;
    ...
    /*
       ...
       {
    //if(i == 1020)
    if(yPlaneUp[i][j] !=((unsigned char*) dstImage)[i* 2048 +j])
    {
    printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)yPlaneUp[i][j], (int)((unsigned char*) dstImage)[i* 2048 +j]);
    //printf("%u ",yPlaneDown[i][j]);
    ...
    */
...

    {
        ...
        for(int i = 0; i< 20; i++)
        {
            ...
            checkErr(status, "clSetKernelArg");
            size_t dstImageDstWidth = 2048;
            size_t dstImageDstHeight = 2048;
...
void Sharpness(unsigned char (*yPlane)[2048][2048], unsigned char (*yPlaneCSER)[2048][2048], unsigned char (*dst)[2048][2048], MMC_CSE_PARAMETER_PUBLIC &cseParamPublic) {

    printf("rs\n");

    short** pEdge1=allocs(2048,2048);

    unsigned char **src=allocu(2048,2048);
    unsigned char **srcCSER=allocu(2048,2048);
    unsigned char **dst1=allocu(2048,2048);
    //unsigned char** yPlaneUp=allocu(2048,2048);
    ...
    for(int i=0;i<2048;i++)
        for(int j=0;j<2048;j++)
        {
            src[i][j]=((unsigned char*)yPlane)[i*2048+j];
            srcCSER[i][j]= ((unsigned char*)yPlaneCSER)[i*2048+j]; 
        }

    printf("ss\n");
    Sharpness_cpu(src, srcCSER,2048,2048,&cseParamPublic, dst1, pEdge1);

    //unsigned short pEdge[2048][2048];
    unsigned short* pEdge = (unsigned short*)malloc(2048*2048*sizeof(short));
...

    {
        ...
        for(int i = 0; i< 20; i++)
        {
            ...
            size_t yPlaneSrcWidth = 2048;
            size_t yPlaneSrcHeight = 2048;
            ...
            checkErr(status, "clSetKernelArg");
            size_t pEdgeDstWidth = 2048;
            size_t pEdgeDstHeight = 2048;
    ...
    for(int i = 0; i < 2048; i++)
    {
        ...
        {
            //if(i == 1020)
            if(pEdge1[i][j] !=((unsigned short*) pEdge)[i* 2048 +j])
            {
                printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)pEdge1[i][j], (int)((unsigned short*) pEdge)[i* 2048 +j]);
                //printf("%u ",yPlaneDown[i][j]);
...

    {
        ...
        for(int i = 0; i < 20; i++)
        {
            ...
            size_t pEdgeSrcWidth = 2048;
            size_t pEdgeSrcHeight = 2048;
            ...
            printf("mean : %f ms\n",tem/20);
    short mean = (item[0]+ 2048*512) / (2048 * 2048);;
...

    {
        ...
        for(int i = 0; i< 20; i++)
        {
            ...
            size_t yPlaneCSERSrcWidth = 2048;
            size_t yPlaneCSERSrcHeight = 2048;
            ...
            checkErr(status, "clSetKernelArg");
            size_t dstDstWidth = 2048;
            size_t dstDstHeight = 2048;
    ...
    for(int i = 0; i < 2048; i++)
    {
        ...
        {
            //if(i == 1020)
            if(abs(dst1[i][j] -((unsigned char*) dst)[i* 2048 +j]) > 1)
            {
                printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)dst1[i][j], (int)((unsigned char*) dst)[i* 2048 +j]);
                //printf("%u ",yPlaneDown[i][j]);
...
void sharpness_total(unsigned char (*yPlane)[2048][2048], unsigned char (*dst)[2048][2048]) {
    ...
    cseParamPublic.peak = 2.F;
    //TODO
    unsigned char* buffer = (unsigned char *)malloc(2048*2048*sizeof(unsigned char));
    unsigned char* buffer1 = (unsigned char *)malloc((2048/4)*(2048/4)*sizeof(unsigned char));
    unsigned char (*yPlaneCSER)[2048][2048] = (unsigned char (*)[2048][2048])buffer;
    unsigned char (*yPlaneDown)[512][512] = (unsigned char (*)[512][512])buffer1;
...
int main() {
    ...
    unsigned char *src_data = (unsigned char *)malloc(sizeof(unsigned char) * 2048 * 2048);
    unsigned char *dst_data = (unsigned char *)malloc(sizeof(unsigned char) * 2048 * 2048);
    for (int i = 0; i < 2048 * 2048; i++) {
        src_data[i] = rand() % 100 + 1;
    }
    unsigned char (*yPlane)[2048][2048] = (unsigned char (*)[2048][2048])src_data;
    unsigned char (*dst)[2048][2048] = (unsigned char (*)[2048][2048])dst_data;
