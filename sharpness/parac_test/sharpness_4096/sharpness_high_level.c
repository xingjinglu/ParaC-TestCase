...
int DownScaleNewX16(unsigned char (*pImageSrc)[4096][4096], unsigned char (*pImageDst)[4096 / 4][4096 / 4]) {

    unsigned char **yPlane=allocu(4096,4096);
    unsigned char** yPlaneDown=allocu(1024,1024);

    for(int i=0;i<4096;i++)
        for(int j=0;j<4096;j++)
            yPlane[i][j]=((unsigned char*)pImageSrc)[i*4096+j];

    printf("ss\n");
    DownScaleNewX16_cpu(yPlane,1024, 1024,  yPlaneDown);

    {
        tem = 0;
        for(int i = 0; i< 20; i++)
        {
            size_t pImageSrcSrcWidth = 4096;
            size_t pImageSrcSrcHeight = 4096;
            ...
            checkErr(status, "clSetKernelArg");
            size_t pImageDstDstWidth = 1024;
            size_t pImageDstDstHeight = 1024;
...
int UpScaleNewX16(unsigned char (*srcImage)[4096 / 4][4096 / 4], unsigned char (*dstImage)[4096][4096]) {

    unsigned char **yPlane=allocu(1024,1024);
    unsigned char** yPlaneUp=allocu(4096,4096);

    for(int i=0;i<1024;i++)
        for(int j=0;j<1024;j++)
            yPlane[i][j]=((unsigned char*)srcImage)[i*1024+j];

    printf("ss\n");
    UpScaleNewX16_cpu(yPlane,1024, 1024, yPlaneUp);
...

    {
        ...
        for(int i = 0; i< 20 ; i++)
        {
            size_t srcImageSrcWidth = 1024;
            size_t srcImageSrcHeight = 1024;
            ...
            size_t dstImageDSWidth = 4096;
            size_t dstImageDSHeight = 4096;
    ...
    /*
       ...
       {
    //if(i == 1020)
    if(yPlaneUp[i][j] !=((unsigned char*) dstImage)[i* 4096 +j])
    {
    printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)yPlaneUp[i][j], (int)((unsigned char*) dstImage)[i* 4096 +j]);
    //printf("%u ",yPlaneDown[i][j]);
    ...
    */
...

    {
        ...
        for(int i = 0; i< 20; i++)
        {
            ...
            checkErr(status, "clSetKernelArg");
            size_t dstImageDstWidth = 4096;
            size_t dstImageDstHeight = 4096;
...
void Sharpness(unsigned char (*yPlane)[4096][4096], unsigned char (*yPlaneCSER)[4096][4096], unsigned char (*dst)[4096][4096], MMC_CSE_PARAMETER_PUBLIC &cseParamPublic) {

    printf("rs\n");

    short** pEdge1=allocs(4096,4096);

    unsigned char **src=allocu(4096,4096);
    unsigned char **srcCSER=allocu(4096,4096);
    unsigned char **dst1=allocu(4096,4096);
    //unsigned char** yPlaneUp=allocu(4096,4096);
    ...
    for(int i=0;i<4096;i++)
        for(int j=0;j<4096;j++)
        {
            src[i][j]=((unsigned char*)yPlane)[i*4096+j];
            srcCSER[i][j]= ((unsigned char*)yPlaneCSER)[i*4096+j]; 
        }

    printf("ss\n");
    Sharpness_cpu(src, srcCSER,4096,4096,&cseParamPublic, dst1, pEdge1);

    //unsigned short pEdge[4096][4096];
    unsigned short* pEdge = (unsigned short*)malloc(4096*4096*sizeof(short));
...

    {
        ...
        for(int i = 0; i< 20; i++)
        {
            ...
            size_t yPlaneSrcWidth = 4096;
            size_t yPlaneSrcHeight = 4096;
            ...
            checkErr(status, "clSetKernelArg");
            size_t pEdgeDstWidth = 4096;
            size_t pEdgeDstHeight = 4096;
    ...
    for(int i = 0; i < 4096; i++)
    {
        ...
        {
            //if(i == 1020)
            if(pEdge1[i][j] !=((unsigned short*) pEdge)[i* 4096 +j])
            {
                printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)pEdge1[i][j], (int)((unsigned short*) pEdge)[i* 4096 +j]);
                //printf("%u ",yPlaneDown[i][j]);
...

    {
        ...
        for(int i = 0; i < 20; i++)
        {
            ...
            size_t pEdgeSrcWidth = 4096;
            size_t pEdgeSrcHeight = 4096;
            ...
            printf("mean : %f ms\n",tem/20);
    short mean = (item[0]+ 4096*4096/2) / (4096 * 4096);;
...

    {
        ...
        for(int i = 0; i< 20; i++)
        {
            ...
            size_t yPlaneCSERSrcWidth = 4096;
            size_t yPlaneCSERSrcHeight = 4096;
            ...
            checkErr(status, "clSetKernelArg");
            size_t dstDstWidth = 4096;
            size_t dstDstHeight = 4096;
    ...
    for(int i = 0; i < 4096; i++)
    {
        ...
        {
            //if(i == 1020)
            if(abs(dst1[i][j] -((unsigned char*) dst)[i* 4096 +j]) > 1)
            {
                printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)dst1[i][j], (int)((unsigned char*) dst)[i* 4096 +j]);
                //printf("%u ",yPlaneDown[i][j]);
...
void sharpness_total(unsigned char (*yPlane)[4096][4096], unsigned char (*dst)[4096][4096]) {
    ...
    cseParamPublic.peak = 2.F;
    //TODO
    unsigned char* buffer = (unsigned char *)malloc(4096*4096*sizeof(unsigned char));
    unsigned char* buffer1 = (unsigned char *)malloc((4096/4)*(4096/4)*sizeof(unsigned char));
    unsigned char (*yPlaneCSER)[4096][4096] = (unsigned char (*)[4096][4096])buffer;
    unsigned char (*yPlaneDown)[1024][1024] = (unsigned char (*)[1024][1024])buffer1;
...
int main() {
    ...
    unsigned char *src_data = (unsigned char *)malloc(sizeof(unsigned char) * 4096 * 4096);
    unsigned char *dst_data = (unsigned char *)malloc(sizeof(unsigned char) * 4096 * 4096);
    for (int i = 0; i < 4096 * 4096; i++) {
        src_data[i] = rand() % 100 + 1;
    }
    unsigned char (*yPlane)[4096][4096] = (unsigned char (*)[4096][4096])src_data;
    unsigned char (*dst)[4096][4096] = (unsigned char (*)[4096][4096])dst_data;
