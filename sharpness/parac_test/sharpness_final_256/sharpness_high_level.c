...
int DownScaleNewX16(unsigned char (*pImageSrc)[256][256], unsigned char (*pImageDst)[256 / 4][256 / 4]) {

    unsigned char **yPlane=allocu(256,256);
    unsigned char** yPlaneDown=allocu(64,64);

    for(int i=0;i<256;i++)
        for(int j=0;j<256;j++)
            yPlane[i][j]=((unsigned char*)pImageSrc)[i*256+j];

    printf("ss\n");
    DownScaleNewX16_cpu(yPlane,64, 64,  yPlaneDown);

    {
        tem = 0;
        for(int i = 0; i< 20; i++)
        {
            size_t pImageSrcSrcWidth = 256;
            size_t pImageSrcSrcHeight = 256;
            ...
            checkErr(status, "clSetKernelArg");
            size_t pImageDstDstWidth = 64;
            size_t pImageDstDstHeight = 64;
   ...
   printf("Down : %f ms\n",tem/20);
    ...
    for(int i = 0; i < 64; i++)
    {
        for(int j = 0; j < 64; j++)
        {
            if(yPlaneDown[i][j] !=((unsigned char*) pImageDst)[i* 64 +j])
            {
                printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)yPlaneDown[i][j], (int)((unsigned char*) pImageDst)[i* 64 +j]);
...
int UpScaleNewX16(unsigned char (*srcImage)[256 / 4][256 / 4], unsigned char (*dstImage)[256][256]) {

    unsigned char **yPlane=allocu(64,64);
    unsigned char** yPlaneUp=allocu(256,256);

    for(int i=0;i<64;i++)
        for(int j=0;j<64;j++)
            yPlane[i][j]=((unsigned char*)srcImage)[i*64+j];

    printf("ss\n");
    UpScaleNewX16_cpu(yPlane,64, 64, yPlaneUp);
...

    {
        ...
        for(int i = 0; i< 20 ; i++)
        {
            size_t srcImageSrcWidth = 64;
            size_t srcImageSrcHeight = 64;
            ...
            size_t dstImageDSWidth = 256;
            size_t dstImageDSHeight = 256;
    ...
    /*
       ...
       {
    //if(i == 1020)
    if(yPlaneUp[i][j] !=((unsigned char*) dstImage)[i* 256 +j])
    {
    printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)yPlaneUp[i][j], (int)((unsigned char*) dstImage)[i* 256 +j]);
    //printf("%u ",yPlaneDown[i][j]);
    ...
    */
...

    {
        ...
        for(int i = 0; i< 20; i++)
        {
            ...
            checkErr(status, "clSetKernelArg");
            size_t dstImageDstWidth = 256;
            size_t dstImageDstHeight = 256;
...
void Sharpness(unsigned char (*yPlane)[256][256], unsigned char (*yPlaneCSER)[256][256], unsigned char (*dst)[256][256], MMC_CSE_PARAMETER_PUBLIC &cseParamPublic) {

    short** pEdge1=allocs(256,256);

    unsigned char **src=allocu(256,256);
    unsigned char **srcCSER=allocu(256,256);
    unsigned char **dst1=allocu(256,256);
    //unsigned char** yPlaneUp=allocu(256,256);

    for(int i=0;i<256;i++)
        for(int j=0;j<256;j++)
        {
            src[i][j]=((unsigned char*)yPlane)[i*256+j];
            srcCSER[i][j]= ((unsigned char*)yPlaneCSER)[i*256+j]; 
        }

    printf("ss\n");
    Sharpness_cpu(src, srcCSER,256,256,&cseParamPublic, dst1, pEdge1);

    unsigned short pEdge[256][256];
...

    {
        ...
        for(int i = 0; i< 20; i++)
        {
            ...
            size_t yPlaneSrcWidth = 256;
            size_t yPlaneSrcHeight = 256;
            ...
            checkErr(status, "clSetKernelArg");
            size_t pEdgeDstWidth = 256;
            size_t pEdgeDstHeight = 256;
    ...
    for(int i = 0; i < 256; i++)
    {
        ...
        {
            //if(i == 1020)
            if(pEdge1[i][j] !=((unsigned short*) pEdge)[i* 256 +j])
            {
                printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)pEdge1[i][j], (int)((unsigned short*) pEdge)[i* 256 +j]);
                //printf("%u ",yPlaneDown[i][j]);
...

    {
        ...
        for(int i = 0; i < 20; i++)
        {
            ...
            size_t pEdgeSrcWidth = 256;
            size_t pEdgeSrcHeight = 256;
            ...
            printf("mean : %f ms\n",tem/20);
    short mean = (item[0]+ 256*256/2) / (256 * 256);;
...

    {
        ...
        for(int i = 0; i< 20; i++)
        {
            ...
            size_t yPlaneCSERSrcWidth = 256;
            size_t yPlaneCSERSrcHeight = 256;
            ...
            checkErr(status, "clSetKernelArg");
            size_t dstDstWidth = 256;
            size_t dstDstHeight = 256;
    ...
    for(int i = 0; i < 256; i++)
    {
        ...
        {
            //if(i == 1020)
            if(abs(dst1[i][j] -((unsigned char*) dst)[i* 256 +j]) > 1)
            {
                printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)dst1[i][j], (int)((unsigned char*) dst)[i* 256 +j]);
                //printf("%u ",yPlaneDown[i][j]);
...
void sharpness_total(unsigned char (*yPlane)[256][256], unsigned char (*dst)[256][256]) {
    MMC_CSE_PARAMETER_PUBLIC cseParamPublic;
    cseParamPublic.cutoff2 = 256.F;
    cseParamPublic.cutoff3Coef = 1.F;
    cseParamPublic.cutoffCoef = 1.F;
    cseParamPublic.peak = 2.F;
    unsigned char yPlaneCSER[256][256];
    unsigned char yPlaneDown[256 / 4][256 / 4];
...
int main() {
    ...
    unsigned char *dst_data = (unsigned char *)malloc(sizeof(unsigned char) * 256 * 256);
    for (int i = 0; i < 256 * 256; i++) {
        src_data[i] = rand() % 100 + 1;
    }
    unsigned char (*yPlane)[256][256] = (unsigned char (*)[256][256])src_data;
    unsigned char (*dst)[256][256] = (unsigned char (*)[256][256])dst_data;
