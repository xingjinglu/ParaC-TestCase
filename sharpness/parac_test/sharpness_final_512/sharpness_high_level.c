...
int DownScaleNewX16(unsigned char (*pImageSrc)[512][512], unsigned char (*pImageDst)[512 / 4][512 / 4]) {

    unsigned char **yPlane=allocu(512,512);
    unsigned char** yPlaneDown=allocu(128,128);

    for(int i=0;i<512;i++)
        for(int j=0;j<512;j++)
            yPlane[i][j]=((unsigned char*)pImageSrc)[i*512+j];

    printf("ss\n");
    DownScaleNewX16_cpu(yPlane,128, 128,  yPlaneDown);

    {
        tem = 0;
        for(int i = 0; i< 20; i++)
        {
            size_t pImageSrcSrcWidth = 512;
            size_t pImageSrcSrcHeight = 512;
            ...
            checkErr(status, "clSetKernelArg");
            size_t pImageDstDstWidth = 128;
            size_t pImageDstDstHeight = 128;
   ...
   printf("Down : %f ms\n",tem/20);
    ...
    for(int i = 0; i < 128; i++)
    {
        for(int j = 0; j < 128; j++)
        {
            if(yPlaneDown[i][j] !=((unsigned char*) pImageDst)[i* 128 +j])
            {
                printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)yPlaneDown[i][j], (int)((unsigned char*) pImageDst)[i* 128 +j]);
...
int UpScaleNewX16(unsigned char (*srcImage)[512 / 4][512 / 4], unsigned char (*dstImage)[512][512]) {

    unsigned char **yPlane=allocu(128,128);
    unsigned char** yPlaneUp=allocu(512,512);

    for(int i=0;i<128;i++)
        for(int j=0;j<128;j++)
            yPlane[i][j]=((unsigned char*)srcImage)[i*128+j];

    printf("ss\n");
    UpScaleNewX16_cpu(yPlane,128, 128, yPlaneUp);
...

    {
        ...
        for(int i = 0; i< 20 ; i++)
        {
            size_t srcImageSrcWidth = 128;
            size_t srcImageSrcHeight = 128;
            ...
            size_t dstImageDSWidth = 512;
            size_t dstImageDSHeight = 512;
    ...
    /*
       ...
       {
    //if(i == 1020)
    if(yPlaneUp[i][j] !=((unsigned char*) dstImage)[i* 512 +j])
    {
    printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)yPlaneUp[i][j], (int)((unsigned char*) dstImage)[i* 512 +j]);
    //printf("%u ",yPlaneDown[i][j]);
    ...
    */
...

    {
        ...
        for(int i = 0; i< 20; i++)
        {
            ...
            checkErr(status, "clSetKernelArg");
            size_t dstImageDstWidth = 512;
            size_t dstImageDstHeight = 512;
...
void Sharpness(unsigned char (*yPlane)[512][512], unsigned char (*yPlaneCSER)[512][512], unsigned char (*dst)[512][512], MMC_CSE_PARAMETER_PUBLIC &cseParamPublic) {

    short** pEdge1=allocs(512,512);

    unsigned char **src=allocu(512,512);
    unsigned char **srcCSER=allocu(512,512);
    unsigned char **dst1=allocu(512,512);
    //unsigned char** yPlaneUp=allocu(512,512);

    for(int i=0;i<512;i++)
        for(int j=0;j<512;j++)
        {
            src[i][j]=((unsigned char*)yPlane)[i*512+j];
            srcCSER[i][j]= ((unsigned char*)yPlaneCSER)[i*512+j]; 
        }

    printf("ss\n");
    Sharpness_cpu(src, srcCSER,512,512,&cseParamPublic, dst1, pEdge1);

    unsigned short pEdge[512][512];
...

    {
        ...
        for(int i = 0; i< 20; i++)
        {
            ...
            size_t yPlaneSrcWidth = 512;
            size_t yPlaneSrcHeight = 512;
            ...
            checkErr(status, "clSetKernelArg");
            size_t pEdgeDstWidth = 512;
            size_t pEdgeDstHeight = 512;
    ...
    for(int i = 0; i < 512; i++)
    {
        ...
        {
            //if(i == 1020)
            if(pEdge1[i][j] !=((unsigned short*) pEdge)[i* 512 +j])
            {
                printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)pEdge1[i][j], (int)((unsigned short*) pEdge)[i* 512 +j]);
                //printf("%u ",yPlaneDown[i][j]);
...

    {
        ...
        for(int i = 0; i < 20; i++)
        {
            ...
            size_t pEdgeSrcWidth = 512;
            size_t pEdgeSrcHeight = 512;
            ...
            printf("mean : %f ms\n",tem/20);
    short mean = (item[0]+ 512*512/2) / (512 * 512);;
...

    {
        ...
        for(int i = 0; i< 20; i++)
        {
            ...
            size_t yPlaneCSERSrcWidth = 512;
            size_t yPlaneCSERSrcHeight = 512;
            ...
            checkErr(status, "clSetKernelArg");
            size_t dstDstWidth = 512;
            size_t dstDstHeight = 512;
    ...
    for(int i = 0; i < 512; i++)
    {
        ...
        {
            //if(i == 1020)
            if(abs(dst1[i][j] -((unsigned char*) dst)[i* 512 +j]) > 1)
            {
                printf("i= %d, j = %d, a = %d, b=%d\n",i, j,(int)dst1[i][j], (int)((unsigned char*) dst)[i* 512 +j]);
                //printf("%u ",yPlaneDown[i][j]);
...
void sharpness_total(unsigned char (*yPlane)[512][512], unsigned char (*dst)[512][512]) {
    MMC_CSE_PARAMETER_PUBLIC cseParamPublic;
    cseParamPublic.cutoff2 = 256.F;
    cseParamPublic.cutoff3Coef = 1.F;
    cseParamPublic.cutoffCoef = 1.F;
    cseParamPublic.peak = 2.F;
    unsigned char yPlaneCSER[512][512];
    unsigned char yPlaneDown[512 / 4][512 / 4];
...
int main() {
    ...
    unsigned char *dst_data = (unsigned char *)malloc(sizeof(unsigned char) * 512 * 512);
    for (int i = 0; i < 512 * 512; i++) {
        src_data[i] = rand() % 100 + 1;
    }
    unsigned char (*yPlane)[512][512] = (unsigned char (*)[512][512])src_data;
    unsigned char (*dst)[512][512] = (unsigned char (*)[512][512])dst_data;
