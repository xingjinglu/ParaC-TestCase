...
#endif
{
...
#ifdef TIME_PAR
  struct timeval time,time0,time0_1,time1,time2,time2_1,time3,time4,time5,time6;
  cl_ulong startTime,endTime,kernelExecTime0,kernelExecTime1,kernelExecTime2,kernelExecTime3,kernelExecTime4,kernelExecTime5;
  ...
  kernelExecTime0 = (endTime-startTime)/1000;
#endif
...
#ifdef TIMEDAY_PAR
  t.env_init_time=(double)(time0.tv_usec-time.tv_usec)*0.001+(time0.tv_sec-time.tv_sec)*1000;
  t.pad_time=(double)(time0_1.tv_usec-time0.tv_usec)*0.001+(time0_1.tv_sec-time0.tv_sec)*1000;
	t.buffer_init_time=(double)(time1.tv_usec-time0_1.tv_usec)*0.001+(time1.tv_sec-time0_1.tv_sec)*1000;
  t.down_time=(double)(time2.tv_usec-time1.tv_usec)*0.001+(time2.tv_sec-time1.tv_sec)*1000;
  t.up_CPU_time=(double)(time2_1.tv_usec-time2.tv_usec)*0.001+(time2_1.tv_sec-time2.tv_sec)*1000;
  t.up_GPU_time=(double)(time3.tv_usec-time2_1.tv_usec)*0.001+(time3.tv_sec-time2_1.tv_sec)*1000;
  t.sobel_time=(double)(time4.tv_usec-time3.tv_usec)*0.001+(time4.tv_sec-time3.tv_sec)*1000;
  t.mean_time=(double)(time5.tv_usec-time4.tv_usec)*0.001+(time5.tv_sec-time4.tv_sec)*1000;
  t.sharp_time=(double)(time6.tv_usec-time5.tv_usec)*0.001+(time6.tv_sec-time5.tv_sec)*1000;
  t.total_used = (double)(time6.tv_usec-time.tv_usec)*0.001+(time6.tv_sec-time.tv_sec)*1000;
#endif

#ifdef KERNELEXECUTION_PAR
  t.kernel_down      = (double)(kernelExecTime0*0.001);
  t.kernel_upborder  = (double)(kernelExecTime1*0.001);
  t.kernel_up        = (double)(kernelExecTime2*0.001);
  t.kernel_sobel     = (double)(kernelExecTime3*0.001);
  t.kernel_mean      = (double)(kernelExecTime4*0.001);
  t.kernel_sharpness = (double)(kernelExecTime5*0.001);
#endif

  t.exe_time= (double)(time6.tv_usec-time0.tv_usec)*0.001+(time6.tv_sec-time0.tv_sec)*1000;
#endif  
...
int main(int argc,char* argv[])
{
    ...
    for(int count=0; count<=0; count++)
    {
...
#ifdef TIMEDAY_PAR
            sum.env_init_time += temp.env_init_time;
            sum.pad_time += temp.pad_time;
            sum.buffer_init_time += temp.buffer_init_time;
            sum.down_time += temp.down_time;
            sum.up_CPU_time += temp.up_CPU_time;
            sum.up_GPU_time += temp.up_GPU_time;
            sum.sobel_time += temp.sobel_time;
            sum.mean_time += temp.mean_time;
            sum.sharp_time += temp.sharp_time;
            sum.total_used += temp.total_used;
#endif
            sum.exe_time += temp.exe_time;

#ifdef KERNELEXECUTION_PAR
            sum.kernel_down      += temp.kernel_down;
            sum.kernel_upborder  += temp.kernel_upborder;
...
#ifdef TIMEDAY_PAR
        avg.env_init_time = sum.env_init_time/testNumber;
        avg.pad_time = sum.pad_time/testNumber;
        avg.buffer_init_time = sum.buffer_init_time/testNumber;
        avg.down_time = sum.down_time/testNumber;
        avg.up_CPU_time = sum.up_CPU_time/testNumber;
        avg.up_GPU_time = sum.up_GPU_time/testNumber;
        ...
        avg.exe_time = sum.exe_time/testNumber;
         ...
         avg.kernel_upborder  = sum.kernel_upborder/testNumber;
         ...
         printf("round%d:%d %d\n",count,height,width);
#ifdef TIMEDAY_PAR
        printf("gettimeofday:\n%f\n%f\n%f\n%f\n%f\n%f\n%f\n%f\n%f\n%f\n%f\n",avg.total_used,avg.env_init_time,avg.exe_time,avg.pad_time,avg.buffer_init_time,avg.down_time,avg.up_CPU_time,avg.up_GPU_time,avg.sobel_time,avg.mean_time,avg.sharp_time);
#endif

#ifdef EXEC_PAR
        printf("kernel execute time:\n%f\n",avg.exe_time);
#endif

#ifdef KERNELEXECUTION_PAR
        printf("clEventGetProfilingInfo:\n%f\n%f\n%f\n%f\n%f\n%f\n",avg.kernel_down,avg.kernel_upborder,avg.kernel_up,avg.kernel_sobel,avg.kernel_mean,avg.kernel_sharpness);
#endif
