...
void OvershootControlFloatX16(unsigned char *src, short *ySharpen,
  ...
  float cutoff2Order = pow(cutoff, 2 * order);
  __m128 cutoff2Order_4 =
      _mm_set_ps(cutoff2Order, cutoff2Order, cutoff2Order, cutoff2Order);
  __m128 cutoff2OrderPeak_4 =
      _mm_set_ps(cutoff2Order * peak, cutoff2Order * peak, cutoff2Order * peak,
                 cutoff2Order * peak);
...
*/
      ...
      __m128 enhanced_4 = _mm_cvtpi16_ps(
          _mm_movepi64_pi64(_mm_loadl_epi64((__m128i *)pySharpen)));
      ...
      __m128i maxValue_8 =
          _mm_max_epu8(pSrc0_8, _mm_max_epu8(pSrc1_8, pSrc2_8));

      __m128 oscStrength_4 = _mm_mul_ps(edge2Order_4, low_4);

      __m128i minValue_8 =
          _mm_min_epu8(pSrc0_8, _mm_min_epu8(pSrc1_8, pSrc2_8));

      oscStrength_4 = _mm_add_ps(oscStrength_4, cutoff2OrderPeak_4);

      maxValue_8 =
          _mm_max_epu8(_mm_srli_si128(maxValue_8, 2),
                       _mm_max_epu8(_mm_srli_si128(maxValue_8, 1), maxValue_8));
      minValue_8 =
          _mm_min_epu8(_mm_srli_si128(minValue_8, 2),
                       _mm_min_epu8(_mm_srli_si128(minValue_8, 1), minValue_8));

      oscStrength_4 =
          _mm_div_ps(oscStrength_4, _mm_add_ps(cutoff2Order_4, edge2Order_4));
      ...
      for (int t = 0; t < 4; t++) {
              ...
              if(enhanced>maxValue)
              {
                      *pic = MIN2(maxValue + oscStrength_4p[t] *
      (enhanced-maxValue),255);
              }
              else if(enhanced<minValue)
              {
                      *pic = MAX2(0, minValue - (oscStrength_4p[t] *
      (minValue-enhanced)));
              }
              else
              {
      ...
      __m128 other = _mm_and_ps(_mm_cmpge_ps(enhanced_4, minValue_4),
                                _mm_cmple_ps(enhanced_4, maxValue_4));

      __m128 pic_temp_4 = _mm_add_ps(
          _mm_mul_ps(_mm_sub_ps(enhanced_4, maxValue_4), oscStrength_4),
          maxValue_4);
      __m128 pic_temp1_4 =
          _mm_sub_ps(minValue_4, _mm_mul_ps(_mm_sub_ps(minValue_4, enhanced_4),
                                            oscStrength_4));

      pic_temp_4 = _mm_and_ps(pic_temp_4, _mm_cmpgt_ps(enhanced_4, maxValue_4));

      pic_temp1_4 =
          _mm_and_ps(pic_temp1_4, _mm_cmplt_ps(enhanced_4, minValue_4));

      pic_temp_4 = _mm_or_ps(_mm_or_ps(pic_temp_4, pic_temp1_4),
                             _mm_and_ps(enhanced_4, other));
...
*/
      ...
      oscStrength = (peak * cutoff2Order + low * edge2Order) /
                    (cutoff2Order + edge2Order);
...
short MeanX16(short *pEdge, int width, int height) {
  ...
  */
  /*LGL SIMD*/
  __m128i r, part_sum;
  part_sum = _mm_set_epi32(0, 0, 0, 0);

  for (i = 0; i < size - 4 + 1; i += 4) {
    r = _mm_lddqu_si128((__m128i *)p);
    r = _mm_cvtepu16_epi32(r);
    part_sum = _mm_add_epi32(part_sum, r);
    p += 4;
  }
  sum = _mm_extract_epi32(part_sum, 3) + _mm_extract_epi32(part_sum, 2) +
        _mm_extract_epi32(part_sum, 1) + _mm_extract_epi32(part_sum, 0);
  while (i < size) {
    sum += *p;
    i++;
    p++;
  ...
  for (int h = 0; h < height; h++) {
    for (int w = 0; w < width; w += 4) {
      ...
      */
       ...
       * Serial*/
      /*
      weight = 1 / (1 + (pEdge[h*width+w] / cutoff2) * (pEdge[h*width+w] /
  cutoff2) * (pEdge[h*width+w] / cutoff2) * (pEdge[h*width+w] / cutoff2));
  strength = (peak - 1.0) * (1 - 1 / (1 + (pEdge[h*width+w] / cutoff) *
  (pEdge[h*width+w] / cutoff) * (pEdge[h*width+w] / cutoff) * (pEdge[h*width+w]
  / cutoff)));
...
*/
      ...
      __m128i yPlaneCSER_4i =
          _mm_loadl_epi64((__m128i *)(*(yPlaneCSER + h) + w));
...
int DownScaleNewX16_1(unsigned char *pImageSrc, int nHeight, int nWidth,
                      unsigned char *pImageDst) {
  int i = 0, j = 0;
...
int DownScaleNewX16(unsigned char *pImageSrc, int nHeight, int nWidth,
  ...
  __m128i r1, r2, r3, r4, part_sum;
  int sum;

  gettimeofday(&start_cpu[0], NULL);
  for (i = 0; i < nHeight; i += 4) {
    for (j = 0; j < nWidth + 1 - 8; j += 8) {
      /*Serial*/
      /*
      // biliner interpolation
      *pDst = (*pSrc1 + *(pSrc1 + 1) + *(pSrc1 + 2) + *(pSrc1 + 3) + *pSrc2 +
      ...
      */
      ...
      /*LGL SIMD*/
      r1 = _mm_lddqu_si128((__m128i *)pSrc1);
      r1 = _mm_cvtepu8_epi16(r1);
      r2 = _mm_lddqu_si128((__m128i *)pSrc2);
      r2 = _mm_cvtepu8_epi16(r2);
      r3 = _mm_lddqu_si128((__m128i *)pSrc3);
      r3 = _mm_cvtepu8_epi16(r3);
      r4 = _mm_lddqu_si128((__m128i *)pSrc4);
      r4 = _mm_cvtepu8_epi16(r4);
      part_sum = _mm_set_epi16(0, 0, 0, 0, 0, 0, 0, 0);
      part_sum = _mm_add_epi16(part_sum, r1);
      part_sum = _mm_add_epi16(part_sum, r2);
      part_sum = _mm_add_epi16(part_sum, r3);
      part_sum = _mm_add_epi16(part_sum, r4);

      sum = (_mm_extract_epi16(part_sum, 3)) +
            (_mm_extract_epi16(part_sum, 2)) +
            (_mm_extract_epi16(part_sum, 1)) + (_mm_extract_epi16(part_sum, 0));

      *pDst = (sum + 8) >> 4;
      pDst++;

      sum = (_mm_extract_epi16(part_sum, 7)) +
            (_mm_extract_epi16(part_sum, 6)) +
            (_mm_extract_epi16(part_sum, 5)) + (_mm_extract_epi16(part_sum, 4));
...
int UpScaleNewX16(unsigned char *srcImage, int height, int width,
  ...
  *(pTmp1 + 3 * dstWidth) = *pTmp1;
  ...
  int wEnd = (width - 1) % 3;
  __m128i mask = _mm_set_epi8(0, 0, 0, 0, 255, 255, 255, 255, 255, 255, 255,
                              255, 255, 255, 255, 255);
  ...
  for (h = 0; h < height - 1; h++) {
    ...
    */
    ...
    for (w = 0; w < wLimit; w++) {
      ...
      __m128 pDstC0l_4 = _mm_add_ps(_mm_mul_ps(p1_4, const_7_8_4),
                                    _mm_mul_ps(p1_r3, const_1_8_4));
      __m128 pDstC0r_4 = _mm_add_ps(_mm_mul_ps(p2_4, const_7_8_4),
                                    _mm_mul_ps(p2_r3, const_1_8_4));
      __m128 pDstC1l_4 = _mm_add_ps(_mm_mul_ps(p1_4, const_5_8_4),
                                    _mm_mul_ps(p1_r3, const_3_8_4));
      __m128 pDstC1r_4 = _mm_add_ps(_mm_mul_ps(p2_4, const_5_8_4),
                                    _mm_mul_ps(p2_r3, const_3_8_4));
      __m128 pDstC2l_4 = _mm_add_ps(_mm_mul_ps(p1_4, const_3_8_4),
                                    _mm_mul_ps(p1_r3, const_5_8_4));
      __m128 pDstC2r_4 = _mm_add_ps(_mm_mul_ps(p2_4, const_3_8_4),
                                    _mm_mul_ps(p2_r3, const_5_8_4));
      __m128 pDstC3l_4 = _mm_add_ps(_mm_mul_ps(p1_4, const_1_8_4),
                                    _mm_mul_ps(p1_r3, const_7_8_4));
      __m128 pDstC3r_4 = _mm_add_ps(_mm_mul_ps(p2_4, const_1_8_4),
                                    _mm_mul_ps(p2_r3, const_7_8_4));

      __m128i pDstC0R0_4 =
          _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(pDstC0l_4, const_7_8_4),
                                      _mm_mul_ps(pDstC0r_4, const_1_8_4)));
      __m128i pDstC0R1_4 =
          _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(pDstC0l_4, const_5_8_4),
                                      _mm_mul_ps(pDstC0r_4, const_3_8_4)));
      __m128i pDstC0R2_4 =
          _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(pDstC0l_4, const_3_8_4),
                                      _mm_mul_ps(pDstC0r_4, const_5_8_4)));
      __m128i pDstC0R3_4 =
          _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(pDstC0l_4, const_1_8_4),
                                      _mm_mul_ps(pDstC0r_4, const_7_8_4)));

      __m128i pDstC1R0_4 =
          _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(pDstC1l_4, const_7_8_4),
                                      _mm_mul_ps(pDstC1r_4, const_1_8_4)));
      __m128i pDstC1R1_4 =
          _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(pDstC1l_4, const_5_8_4),
                                      _mm_mul_ps(pDstC1r_4, const_3_8_4)));
      __m128i pDstC1R2_4 =
          _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(pDstC1l_4, const_3_8_4),
                                      _mm_mul_ps(pDstC1r_4, const_5_8_4)));
      __m128i pDstC1R3_4 =
          _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(pDstC1l_4, const_1_8_4),
                                      _mm_mul_ps(pDstC1r_4, const_7_8_4)));

      __m128i pDstC2R0_4 =
          _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(pDstC2l_4, const_7_8_4),
                                      _mm_mul_ps(pDstC2r_4, const_1_8_4)));
      __m128i pDstC2R1_4 =
          _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(pDstC2l_4, const_5_8_4),
                                      _mm_mul_ps(pDstC2r_4, const_3_8_4)));
      __m128i pDstC2R2_4 =
          _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(pDstC2l_4, const_3_8_4),
                                      _mm_mul_ps(pDstC2r_4, const_5_8_4)));
      __m128i pDstC2R3_4 =
          _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(pDstC2l_4, const_1_8_4),
                                      _mm_mul_ps(pDstC2r_4, const_7_8_4)));

      __m128i pDstC3R0_4 =
          _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(pDstC3l_4, const_7_8_4),
                                      _mm_mul_ps(pDstC3r_4, const_1_8_4)));
      __m128i pDstC3R1_4 =
          _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(pDstC3l_4, const_5_8_4),
                                      _mm_mul_ps(pDstC3r_4, const_3_8_4)));
      __m128i pDstC3R2_4 =
          _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(pDstC3l_4, const_3_8_4),
                                      _mm_mul_ps(pDstC3r_4, const_5_8_4)));
      __m128i pDstC3R3_4 =
          _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(pDstC3l_4, const_1_8_4),
                                      _mm_mul_ps(pDstC3r_4, const_7_8_4)));

      __m128i pDstR0_4 = _mm_or_si128(
          pDstC0R0_4,
          _mm_or_si128(_mm_slli_si128(pDstC1R0_4, 1),
                       _mm_or_si128(_mm_slli_si128(pDstC2R0_4, 2),
                                    _mm_slli_si128(pDstC3R0_4, 3))));
      __m128i pDstR1_4 = _mm_or_si128(
          pDstC0R1_4,
          _mm_or_si128(_mm_slli_si128(pDstC1R1_4, 1),
                       _mm_or_si128(_mm_slli_si128(pDstC2R1_4, 2),
                                    _mm_slli_si128(pDstC3R1_4, 3))));
      __m128i pDstR2_4 = _mm_or_si128(
          pDstC0R2_4,
          _mm_or_si128(_mm_slli_si128(pDstC1R2_4, 1),
                       _mm_or_si128(_mm_slli_si128(pDstC2R2_4, 2),
                                    _mm_slli_si128(pDstC3R2_4, 3))));
      __m128i pDstR3_4 = _mm_or_si128(
          pDstC0R3_4,
          _mm_or_si128(_mm_slli_si128(pDstC1R3_4, 1),
                       _mm_or_si128(_mm_slli_si128(pDstC2R3_4, 2),
                                    _mm_slli_si128(pDstC3R3_4, 3))));
