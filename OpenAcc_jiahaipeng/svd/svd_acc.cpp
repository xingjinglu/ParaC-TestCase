...
void print_matrix(TYPE *A)
{
    ...
    for(int i = 0; i < ROWS; i++)
    {
        for(int j = 0; j < COLS * 2; j++)
            cout << setprecision(5) <<  A[i * COLS * 2 + j] << " ";
        cout << endl;
    }
}
//打印向量  只为调试
...
TYPE inner_product(TYPE *src1, TYPE *src2)
{
    TYPE  real = 0.0f;
    TYPE  imag = 0.0f;

#pragma acc loop reduction(add:real, add:imag)
    for(int i = 0; i < ROWS; i++)
    {
        real += src1[2 * i] * src2[2 * i]     + src1[2 * i + 1] * src2[2 * i + 1];
        imag += src1[2 * i] * src2[2 * i + 1] - src1[2 * i + 1] * src2[2 * i];
    }
...
#pragma acc loop reduction(add:result)
    for(int i = 0; i < ROWS; i++)
...
TYPE part_product(TYPE *src1, TYPE *src2, int is_src1_real, int is_src2_real)
{
    TYPE result = 0.0f;
...
#pragma acc loop reduction(add:result)
        for(int i = 0; i < ROWS; i++)
            result += src1[2 * i] * src2[2 * i];

    if(is_src1_real == 0 && is_src2_real == 0)
#pragma acc loop reduction(add:result)
        for(int i = 0; i < ROWS; i++)
            result += src1[2 * i + 1] * src2[2 * i + 1];

    if(is_src1_real == 1 && is_src2_real == 0)
#pragma acc loop reduction(add:result)
        for(int i = 0; i < ROWS; i++)
            result += src1[2 * i] * src2[2 * i + 1];

    if(is_src1_real == 0 && is_src2_real == 1)
#pragma acc loop reduction(add:result)
        for(int i = 0; i < ROWS; i++)
            result += src1[2 * i + 1] * src2[2 * i];
...
//根据计算结果更新矩阵A的数据
void update_data(TYPE *A, TYPE *ci, TYPE *cj, TYPE sin_x, TYPE cos_x, TYPE sin_alpha, TYPE cos_alpha, int col, int flag)
{
    TYPE real = 0.0f;
    TYPE imag = 0.0f;
...
#pragma acc parallel loog present(A) gang
            for(int j = start_index + 1; j < COLS; j+=2)
            {
                ...
                //判断两列是否正交
                TYPE value = inner_product(ci, cj);

                if(IN_PRODUCT != 1)
                    value = value/norm(ci)/norm(cj); 
                ...
                if(skip_trans ==0)
                {
                    ...
                    TYPE inner_ri = part_product(ci, cj, 1, 0);
                    TYPE inner_ir = part_product(ci, cj, 0, 1);
                    TYPE inner_rr = part_product(ci, cj, 1, 1);
                    TYPE inner_ii = part_product(ci, cj, 0, 0);

                    TYPE a = inner_ri - inner_ir;
                    TYPE b = inner_rr + inner_ii;
                    ...
                    TYPE inner_rrj = part_product(cj, cj, 1, 1);
                    TYPE inner_iij = part_product(cj, cj, 0, 0);
                    TYPE inner_rri = part_product(ci, ci, 1, 1);
                    TYPE inner_iii = part_product(ci, ci, 0, 0);

                    TYPE n = 0.5 * (inner_rrj + inner_iij - inner_rri - inner_iii);
...
int main()
{
    ...
    {
        for(int j = 0; j < COLS; j++)
            f >>setprecision(6) >>  A[i * COLS * 2 + j * 2];

        for(int j = 0; j < COLS; j++)
            f >>setprecision(6) >>  A[i * COLS * 2 + j * 2 + 1];
    }
    ...
    //进行SVD计算
#pragma acc data copyin(A[0:ROWS*COLS*2]) copyout(A[0:ROWS*COLS*2], B[0:ROWS*COLS*2], V[0:ROWS*COLS*2], S[COLS*2]) 
    svd(A, B, S, V);
